...
_KV_SENSITIVE_PATTERN = re.compile(
    r'(?i)\b(password|passwd|pwd|secret|token|api[_-]?key|access[_-]?token|refresh[_-]?token|authorization)\b(\s*[:=]\s*)(["\']?)[^,\s"\'\]\}]+(["\']?)'
)
...
_NAMED_SENSITIVE_KV_PATTERN = re.compile(
    r'(?i)(["\']?[A-Za-z0-9_.-]*(?:token|secret|password|passwd|pwd|api[_-]?key|authorization|credential|private[_-]?key)[A-Za-z0-9_.-]*["\']?\s*[:=]\s*)(["\'])(.*?)(\2)'
)
