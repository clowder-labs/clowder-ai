...
This guide is for **maintainers and triagers** of the OfficeClaw project. If you're looking for how to contribute, see [CONTRIBUTING.md](CONTRIBUTING.md).
...
gh issue list --repo zts212653/office-claw --state open --search "{keywords}"
```
...
```bash
gh issue edit {N} --repo zts212653/office-claw --add-label "triaged"
```

If the issue needs a maintainer decision on direction or scope:

```bash
gh issue edit {N} --repo zts212653/office-claw --add-label "needs-maintainer-decision"
```
...
| **Find your assignments** | `gh issue list --repo zts212653/office-claw --assignee @me` |
...
- `office-claw#58` or `office-claw#120` — unambiguous
...
3. Review the [current label list](https://github.com/zts212653/office-claw/labels) — familiarize yourself with what exists
...
本指南面向 OfficeClaw 项目的 **maintainer 和 triager**。如果你想了解如何贡献代码，请看 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
```bash
gh issue list --repo zts212653/office-claw --state open --search "{关键词}"
```
...
| **查自己的认领** | `gh issue list --repo zts212653/office-claw --assignee @me` |
...
2. 设置仓库的 GitHub 通知
3. 看看[当前标签列表](https://github.com/zts212653/office-claw/labels)，熟悉现有标签
