...
4. materialize the staged `@clowder/shared` package into dependent package `node_modules`
...
- `zod`
- staged `@clowder/shared`

### `packages/shared`

The staged shared package is copied as runtime output and then materialized into dependent `node_modules/@clowder/shared` paths when the package manager creates a symlink.
