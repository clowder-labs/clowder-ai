...
**操作**：这是 workflow-mandated schedule。先调用 `cat_cafe_preview_scheduled_task` 生成 draft，核对下面参数；核对无误后调用 `cat_cafe_register_scheduled_task` 注册，with no extra user confirmation：
...
**注册失败不阻塞清理**：记 telemetry / 留痕后继续 Step 8，事后补注册——调度器故障不该扣押 worktree（旧版 fail-closed 是自噬环）。
