    ...
    private async Task InjectLoginCssAsync()
    {
        ...
        try
        {
            var huaweiScript =
                ...
                "fixForgetPwdLink();" +
                "if(document.readyState!=='complete'){document.addEventListener('DOMContentLoaded',function(){replaceSpans();fixPrivacyLinks();hideElements();fixForgetPwdLink()})}" +
                "new MutationObserver(function(){replaceSpans();fixPrivacyLinks();hideElements();fixForgetPwdLink()}).observe(document.documentElement,{childList:true,subtree:true});" +
                "})();";
