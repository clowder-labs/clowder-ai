    ...
    private async Task InjectLoginCssAsync()
    {
        ...
        try
        {
            var huaweiScript =
                ...
                "fixPrivacyLinks();" +
                "function hideElements(){if(!isHuaweicloud())return;var html=document.documentElement;if(html)html.style.setProperty('min-width','0','important');var idp=document.getElementById('idpLinkDiv');if(idp)idp.style.display='none';var eChannel=document.getElementById('eChannelLinkDiv');if(eChannel)eChannel.style.display='none';var vmall=document.getElementById('vmallLinkDiv');if(vmall)vmall.style.display='none';var idpLogin=document.getElementById('idpLoginLinkDiv');if(idpLogin)idpLogin.style.display='none';var intervals=document.querySelectorAll('#hwAccountLinkDiv ~ .intervalDiv');for(var i=0;i<intervals.length;i++){intervals[i].style.display='none'}}" +
                "hideElements();" +
