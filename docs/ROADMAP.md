...
| F168 | Community Operations Board — 社区事务编排引擎（运营闭环未上线） | infra-complete / ops-gap | Ragdoll Opus-4.8 | internal | [F168](features/F168-community-ops-board.md) |
...
| F258 | BLE Physical Event Limb — 可审计的物理事件总线（复用 F126 控制面；BLE Central / GATT、显式绑定、标准传感器、按钮事件、平台原生 helper；proximity 不作为敏感操作认证） | in-progress (Phase A implementation complete; AC-A8 hardware acceptance + implementation review pending) | Maine Coon Sol (GPT-5.6 Sol) | internal (operator 2026-07-14「立项开搞」) | [F258](features/F258-ble-physical-event-limb.md) |
