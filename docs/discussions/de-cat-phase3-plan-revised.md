# 第三轮术语收口实施计划（Phase 3 - 修订版）

> 日期：2026-04-12
> 目标：收口前两轮遗留的运行时路径、skills 目录、打包暴露面与历史双轨问题，消除旧路径/旧前缀与现行命名并存。
> 关联文档：
...
第三轮不是“把 `office-claw-skills/` 直接删掉”，也不是“只改 4 个路径文件”。
...
2. **skills 资产迁移**：谨慎处理 `office-claw-skills/` 中仍有价值的内容，尤其 `pptx-craft-simple`。
...
5. 安装器/构建链路仍有旧目录名假设：
   - `packages/api/test/windows-offline-installer.test.js` 仍断言 `office-claw-skills`
...
1. 新运行不再创建新的 `.office-claw/...` 项目级路径。
2. 新运行不再创建新的 `catcafe_*` sessionId。
3. 新安装包和运行时主目录不再依赖 `office-claw-skills/` 作为正式技能目录。
4. `office-claw-skills/` 中仍有价值的 skills 内容完成迁移或明确归档。
...
1. 代码仍写 `.office-claw` / `catcafe_*`。
2. `office-claw-skills/` 仍含有效 skills 资产，不能粗暴删除。
...
对于 `office-claw-skills/` 中的内容，判断是否需要保留/迁移，**不能只看代码直接引用**，还要看：
...
3. `office-claw-skills/pptx-craft` 中的 `designer / planner / outline-research / html-to-pptx / assets` 不是可随意删除的旧垃圾，而是 **后续补入旧目录、但未成功收敛进 office 目录的有效依赖资产**。
4. `office-claw-skills/pptx-craft-simple` 是后续新增的一套候选实现，**应保留**，但当前不能原样上线，因为它的 `name`、内部路径和自我引用仍指向 `pptx-craft`，会造成命名与路径冲突。
5. 因此 `pptx-craft` 与 `pptx-craft-simple` 的处理方式不同：
   - `pptx-craft`：以 `office-claw-skills/pptx-craft` 为主，吸收 `office-claw-skills/pptx-craft` 中缺失依赖后继续保留。
...
1. `8918d07b` `feat: add pptx-craft skill`
   - 最早创建 `office-claw-skills/pptx-craft`
...
2. `dda7ac93` `refresh pptx-craft (#204)`
   - 继续更新的仍是 `office-claw-skills/pptx-craft`
   - 重点是脚本与导出链路调整，并非完整多模块版本

3. `55721e54` `feat(F140): Phase M-5a — rename office-claw-skills/ to office-claw-skills/`
   - 这是去猫化 rename 提交
...
4. `4bfe7ca9` `提交新版本pptxskills版本`
   - 在旧目录 `office-claw-skills/` 侧新增 `pptx-craft-simple/`
   - 同时大幅补充 `office-claw-skills/pptx-craft/`，加入：
     - `designer/`
     - `planner/`
     - `outline-research/`
     - `html-to-pptx/`
...
5. 因此当前状态不是单纯“rename 漏了一个目录”，而是：
   ...
   - 最终 rebase/merge 后形成：
     ...
     - `office-claw-skills/pptx-craft` 保留了后来补入的依赖资产
     - `office-claw-skills/pptx-craft-simple` 保留了后来新增的候选实现

这一证据链直接决定了本轮的处理策略：

1. 不能删除 `office-claw-skills/pptx-craft`，因为它承载了正式主 skill 缺失的依赖资产。
2. 不能原样迁移 `pptx-craft-simple`，因为它虽然是新增有效实现，但尚未完成独立命名和路径标准化。
3. 最终应在资产吸收与标准化完成后，彻底删除 `office-claw-skills/`。

### 4.4 因此本轮对 skills 的原则

1. 不直接删除 `office-claw-skills/`。
2. 先迁移确定需要保留的内容。
3. 再让代码/打包链路切换到 `office-claw-skills/`。
4. 最后再决定 `office-claw-skills/` 是归档、兼容保留，还是删除。
...
当前：`join(hostRoot, '.office-claw', OWNER_FILENAME)`
...
当前返回：`.office-claw/skills/${localName}`
...
当前返回：`.office-claw/skills/${skillName}`
...
2. 风险：前端或调用方缓存了 `.office-claw/...` 路径格式，路径返回值切换后出现展示异常、跳转失效或错误提示。
...
- `office-claw-skills/pptx-craft/designer/`
- `office-claw-skills/pptx-craft/planner/`
- `office-claw-skills/pptx-craft/outline-research/`
- `office-claw-skills/pptx-craft/html-to-pptx/`
- `office-claw-skills/pptx-craft/assets/`
...
2. `office-claw-skills/pptx-craft` 不再作为正式 skill 并存，而是作为回补来源被吸收。
3. 迁移完成后，必须核对 `office-claw-skills/pptx-craft/SKILL.md` 中声明的依赖与实际目录一致。

#### 3B.2 `pptx-craft-simple`：标准化后上线

来源：`office-claw-skills/pptx-craft-simple/`
...
#### 3B.3 `office-claw-skills/` 最终处置

`office-claw-skills/` 的最终目标是 **彻底删除**，但前提是完成内容收敛。
...
4. 验证功能和扫描结果后，删除 `office-claw-skills/`
...
本轮不建议把 `office-claw-skills/` 直接加入 `.gitignore` 后作为本地孤儿目录长期存在。
...
2. 再切换代码与打包链路。
3. 再根据验证结果二选一：
   - 方案 1：短期临时保留 `office-claw-skills/` 作为过渡目录，但不再被正式路径使用。
   - 方案 2：验证完成后直接删除旧目录。

不推荐：

1. 一开始就简单 `mv office-claw-skills office-claw-skills.backup`
...
3. 风险：回补 `office-claw-skills/pptx-craft` 资产时，如果无差别整体覆盖 `office-claw-skills/pptx-craft`，可能把去猫化后已修正的内容覆盖回旧状态。
...
4. 风险：过早删除 `office-claw-skills/` 会直接丢失未收敛资产，且此类损失在代码搜索中不一定立刻暴露。
...
1. 无其他历史拷贝逻辑继续复制 `office-claw-skills/`
...
1. `.office-claw/skills/...` → `.office-claw/skills/...`
2. `catcafe_*` → 新 session 前缀

#### 3C.4 风险分析

1. 风险：仓库代码已切换到 `office-claw-skills`，但安装器或离线包仍然复制 `office-claw-skills/`，最终交付物继续带旧目录。
...
3. 风险：安装器文档、脚本和测试不一致，会让后续维护者误把 `office-claw-skills/` 当成仍被支持的目录。
   - 触发条件：只改构建逻辑，不改文档和说明。
...
2. 风险：用户目录和项目目录双轨同时存在时，如果迁移顺序错误，可能刚迁完又被旧代码继续写回 `.office-claw`。
...
1. 解决你当前机器上的并存问题。
2. 防止用户继续看到 `.office-claw` 与 `.office-claw` 并存。
...
9. 最后再决定是否删除 `office-claw-skills/`
...
- [ ] 生产代码默认路径不再写 `.office-claw/skills`
- [ ] 生产代码默认官方技能目录不再指向 `office-claw-skills`
- [ ] API 返回给前端的本地 skill 路径不再出现 `.office-claw`
...
- [ ] 最终 bundle 不再把 `office-claw-skills/` 当正式运行目录
- [ ] 安装包中默认路径说明不再出现旧 skills 目录名

### 8.4 运行时层

- [ ] 新运行只写 `.office-claw`
- [ ] 历史 `.office-claw` 数据不再被继续增量写入
- [ ] 用户目录双轨状态已有明确迁移或保留策略
...
1. 直接删除 `office-claw-skills/`
2. 仅凭“代码搜不到引用”判定某个 skill 无用
3. 先把 `office-claw-skills/` 重命名到 `.backup/` 再慢慢想后续
...
**先让新代码和新安装包只走 `office-claw` 主路径，再把 `office-claw-skills/` 中仍有价值的 skills 资产安全迁移过去，最后处理历史双轨数据。**
