...
> OfficeClaw / OfficeClaw 内如何配置 ACP provider、ACP model profile、provider 环境变量，以及如何把 cat 绑定到外部 ACP agent。

## 概述

OfficeClaw 里的 ACP 接入分成三层：

1. **ACP Provider Profile**
   定义如何启动外部 ACP agent 进程，例如 `relay-teams gateway acp stdio` 或 `opencode acp`，并可选附带一组安全过滤后的环境变量。
2. **ACP Model Profile**
   仅在 `clowder_default_profile` 模式下使用，由 OfficeClaw 把模型参数和密钥下发给 ACP agent。
3. **Cat 绑定**
...
- `mcpCapabilities.stdio`: OfficeClaw 继续按 host-provided stdio MCP server 注入
- `mcpCapabilities.acp`: OfficeClaw 注入 `transport: "acp"` 的 `office-claw` server，并在运行期桥接 `mcp/connect`、`mcp/message`、`mcp/disconnect`
...
OfficeClaw 通过 `resumeCatId` 显式指定“本次消息是恢复哪只猫的中断会话”。该字段会绕过普通 mention 路由，并保持队列隔离，避免把恢复 turn 和普通 follow-up 合并。
...
- OfficeClaw 会在 `session/new` 或 `session/load` 时下发模型配置
...
- OfficeClaw 只负责 ACP 会话，不接管它的 provider / credential 体系
...
如果 provider 运行在 `clowder_default_profile` 模式，还会额外拒绝模型凭据相关前缀，避免 ACP provider 通过自定义 env 覆盖 OfficeClaw 下发的模型配置：
...
这说明 `session/new` 确实带上了 `modelProfileOverride`，而不是只在 OfficeClaw 侧“看起来已配置”。
...
- 默认存储根目录：`$HOME/.office-claw/`
- 如设置了 `CAT_CAFE_GLOBAL_CONFIG_ROOT`，则改为 `<CAT_CAFE_GLOBAL_CONFIG_ROOT>/.office-claw/`
- provider 元信息：`<storageRoot>/.office-claw/provider-profiles.json`
- provider secrets：`<storageRoot>/.office-claw/provider-profiles.secrets.local.json`
- ACP model 元信息：`<storageRoot>/.office-claw/acp-model-profiles.json`
- ACP model secrets：`<storageRoot>/.office-claw/acp-model-profiles.secrets.local.json`

旧的项目内 `.office-claw/provider-profiles.json` 会在读取时自动迁移到全局存储根。
