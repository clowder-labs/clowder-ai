...
OfficeClaw 的开发是**愿景驱动**的。和用户确认了 feature 的愿景后：

- **没达成愿景 = 没完成**，必须继续做，不能半路停下来问"要不要继续"（§17）
- **唯一停下来的理由**：发现了原本没发现的、确实解决不了的阻塞（技术限制/外部依赖不可用），此时升级用户
...
`../office-claw-runtime` 是咱们的运行态单实例（通常占用 `3003/3004`），默认视为**在线服务**，不是随手重启的实验环境。
...
3. 确实要重启，必须先拿到用户明确同意，再显式设置 `CAT_CAFE_RUNTIME_RESTART_OK=1` 执行启动命令

说明：`--force` 不是重启授权，不能替代第 3 条。

## Alpha 验收通道

`../office-claw-alpha` 是基于最新 `origin/main` 的隔离测试环境，供用户和智能体们验收最新改动，不干扰 runtime。
...
```
⓪ Design Gate    → 设计确认（UX→用户/后端→智能体/架构→两边）
① worktree        → 隔离开发环境
...
> **⚠️ 全链路自动推进（§17）！** SOP 有写下一步 → 直接做，不要停下来问用户。

| Step | 做什么 | Skill | 详情 |
|------|--------|-------|------|
| ⓪ | 设计确认：前端→用户画 wireframe；后端→智能体讨论；架构→两边 | `feat-lifecycle` Design Gate | Trivial 跳过⓪，按下方例外路径判断 |
| ① | 创建 worktree，配置 Redis 6398 | `worktree` | 禁止直接改 main |
| ② | 愿景对照 + spec 合规 + 跑测试 + **有 .pen 则设计稿对照** | `quality-gate` | AC ≠ 完成，问"用户体验如何？" |
...
| shared rebuild | `pnpm --filter @office-claw/shared build` | shared 包改后 |
...
开源仓 `office-claw` 的社区 issue 由智能体 triage，**用户决定是否立项**。
...
| **F 号分配** | 用户拍板 → 猫执行 | 在 ROADMAP.md 加条目，分配下一个可用 F 号 |
...
```
社区开 issue → 猫 triage（加 label）→ 用户拍板
    ├─ Feature → ROADMAP.md 加 F{NNN} → Feature Doc → 实现 → 全量 sync 推送
    └─ Bug fix → worktree(sync tag) → 修 → sync-hotfix.sh → office-claw PR → cherry-pick 回 main
```
...
1. `git worktree add -b fix/xxx ../office-claw-hotfix-xxx sync/LATEST-TAG`
2. 在 worktree 里修 bug
3. `cd ../office-claw-hotfix-xxx && bash scripts/sync-hotfix.sh fix/xxx <changed-files>`
4. 在 office-claw 上开 PR、review、merge
5. Cherry-pick fix 回 office-claw main
...
全量同步到 `office-claw` 时，**不能只看家里的 `pnpm gate` 绿不绿**。  
`source gate green != target/public gate green`。

硬规则：
1. 先在 `office-claw` 导出同一份同步产物到 **temp target**
2. 在 temp target 跑完整 public gate：`pnpm check`、`pnpm lint`、`build`、`pnpm --filter @office-claw/api run test:public`、startup acceptance
3. **只有 temp target public gate 全绿，才允许碰真实 `office-claw`**
4. 本机 README/macOS smoke 不属于 full sync 主路径；它必须是 sync 完成后的独立步骤，且必须显式隔离端口/Redis

一句话：**不要再把真实 `office-claw` 当第一轮验收场，更不能把 runtime 当验收靶子。**

### Release Provenance（三点映射）

公开 release 不要求 `office-claw` 和 `office-claw` 同 SHA；我们要求的是**可追溯映射**。
...
- **F 编号唯一源**：ROADMAP.md（用户拍板后猫执行分配）
- **Bug 不编号**：直接用 issue # 追踪，修完 close（D7）
- **贡献者不自选号**：CONTRIBUTING.md 已写明，智能体回复时也要强调（D8）
- **分配 F 号前必须做关联检测**：确认 issue 不是现有 feature 的子项/增强（F114-F116 撤销教训，D9）
- **社区贡献者的 PR**：智能体用 `community-pr` skill 引导（编号校验 + Feature Doc 对齐）

### Issue Label 命名规范

开源仓 `office-claw` 的 issue label 统一格式：

| Label | 格式 | 颜色 | 说明 |
|-------|------|------|------|
| Feature 关联 | `feature:F{NNN}` | `#0E8A16` 绿 | 关联到 office-claw Feature 编号 |
...
- Label 在 office-claw 定义规范，通过 sync 流程同步到 office-claw 的 CONTRIBUTING.md
