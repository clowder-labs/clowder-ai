...
> **Status**: done | **Owner**: Claude（实现）+ Codex（review）
> 日期: 2026-03-05
> 完成日期: 2026-03-06
...
智能体的 session 被封印（context 达到阈值）后，新启动的 session 几乎"失忆"：

1. **智能体崇崇（Task 列表）丢失** — 新猫不知道有哪些任务、做到哪了
...
team experience："Session chain 新启动的猫需要继承过去的猫的智能体崇崇。现在是你之前的 chain 上下文超过了被封印了，然后启动后的新 session 的你，我估计是没自动继承这个 plan 的。"
...
| # | AC | Phase |
|---|-----|-------|
| AC-1 | 新 session bootstrap 包含当前 thread 的 Task 列表（智能体崇崇）快照 | A |
...
|----|---------------------------|---------|----------|------|
| R1 | "新启动的猫需要继承过去的猫的智能体崇崇" | AC-1, AC-2 | test: bootstrap 输出包含 task 列表 | [x] Phase A `e5082209` |
...
| R5 | Bootstrap 不能超预算（Codex review 发现） | AC-5 | test: serial/parallel/incremental 三路径 token cap | [x] Phase A — section-aware cap 2000 tokens |
| R6 | Task 内容不能变成注入攻击（Codex review 发现） | AC-6 | test: 恶意 title/why 截断+转义 | [x] Phase A — sanitize() + 12 injection tests |
...
| KD-5 | ThreadMemory token 上限 `min(3000, floor(maxPromptTokens * 0.03))`，下限 1200 | Codex分析：预算未扣 bootstrap，Spark 64k prompt 下 3% ≈ 1920 |
| KD-6 | Task 快照格式：紧凑列表 + 焦点任务，doing>blocked>todo>done 排序，最多 8 open + 2 done | Codex建议，约 200-400 tokens |
| KD-7 | Task title/why 按数据块渲染，截断 80/120 字符，含注入防护 | Codex P1 安全发现 |
...
- Phase A: Claude实现 → Codex review → PR #229 merged ✅
- Phase B: Claude实现 → Codex review → PR #234 merged ✅
- Phase C: Claude实现 → Codex review → PR #240 merged ✅
- Hotfix (P1 injection + P2-2 input cap): Claude实现 → 云端 codex R1~R8 (6P1+2P2→全清零) → PR #243 merged ✅
