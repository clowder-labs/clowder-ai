...
> **Status**: in-progress (Phase I in progress) | **Owner**: Claude | **Priority**: P1 | **Reopened**: 2026-03-14
>
> **重新打开原因**：2026-03-12 声称 done 并通过愿景守护，但team lead 2026-03-14 实际启动 dev 点开狼人杀后发现：(1) 无关闭/返回按钮，用户被困在全屏游戏界面；(2) 无大厅/配置流程，7 只猫自动塞入无法选择；(3) 智能体不会自动行动，游戏永远卡在 night_guard 等待中；(4) 整体不可用。92 个单元测试全绿但零 E2E 真实验证。教训见 LL-032。
...
四猫讨论收敛（Claude + Gemini + Codex GPT-5.4 + Codex Codex），核心共识：
...
- 归档分类：`游戏-狼人杀`，在左侧栏可快速定位（参考现有 office-claw / studio-flow / 未分类 project 分类体系）
- **KD-18**: 游戏和日常聊天完全隔离，游戏有专属空间

**D2. 智能体身份保留**
- 智能体在游戏内**保留咖啡馆身份**（Claude/Codex/Gemini），不需要新 persona
- 复用现有头像系统（CatAvatar + `/avatars/{catId}.png`）
- **KD-19**: 不需要"玩家3"之类的通用身份，智能体就是智能体

**D3. 上帝操控面板**
- 发牌（手动分配角色）✅
- 暂停/恢复（"我要去上厕所你们总得等等我"）✅
- 跳过当前阶段（帮卡住的局面推进）✅
- ~~踢人~~（team lead："太过分了 智能体做错什么了"）❌
- **KD-20**: 上帝面板三个核心按钮：发牌、暂停/恢复、跳过当前阶段

**D4. 真实到达/就绪状态**
- 展示每只猫的**真实加载状态**，不做假动画
- 卡住的猫要有 loading 指示，team lead担心智能体卡住看不到
- **KD-21**: ready 状态必须反映真实情况

**D5. 狼人智能体风 UX**
- 设计关键词：**可爱 + 暗色调 + 智能体穿狼人服装/装扮**
- 不是纯暗黑 RPG，不是纯可爱，是**智能体 cosplay 狼人**的混搭风格
- team experience："智能体装狼人那种可爱的带点黑色的风格"
- **KD-22**: 视觉风格 = 狼人智能体风（cute dark）
...
- [x] AC-B3: 智能体 AI 玩家能合理发言和执行夜间动作 — ✅ Phase C GameAutoPlayer 修复（PR #454），PR #478 补 hasActed 状态反馈
...
- [x] AC-D2: 智能体保留咖啡馆身份（Claude/Codex/Gemini），复用现有头像系统
- [x] AC-D3: 上帝面板三按钮（发牌、暂停/恢复、跳过阶段），无踢人功能
- [x] AC-D4: 每只猫展示真实 ready 状态 + 卡住时有 loading 指示
- [x] AC-D5: 狼人智能体风 UX（可爱+暗色调+智能体 cosplay 狼人）— 需Gemini参与视觉资产
...
- [x] AC-E1: 上帝推理模式（Detective Mode）— 观战者开局选定一只猫，只能看到该玩家的身份和信息权限，其余座位只看到公开信息。team experience："只能选择一只猫看他身份，狼人杀观战模式那种"
  - 视觉：塔罗牌卡背 + 灵魂链接光效 + 翻牌仪式（Gemini提案）— ⬜ 视觉资产待Gemini
...
- [x] AC-F7: 慢启动智能体有 grace period + god-view 展示真实连接状态
- [x] AC-F8: team lead在 god-view 能清楚理解"正在发生什么"（不再一脸懵逼）

### Phase H6（Chat UI 重做 — 对齐 .pen 设计稿）✅

愿景守护 review by Claude Opus 4.5（2026-03-19）→ 踢回 → 修复 → codex review 放行 → merged
...
| 4 | — | displayName 格式 "Claude(opus)" | ✅ GameViewBuilder.enrichDisplayName via catRegistry |
...
### Phase I（Agent-Driven Game — 智能体真正玩游戏）🚧

team lead 2026-03-20 批评：当前 `GameAutoPlayer` + `LlmAIProvider` 只是裸调 LLM API，智能体根本不知道自己在玩游戏。三猫（OpenCode诊断 + Claude架构 + Codex审查）一致同意重做驱动层。

**P0 前置条件（信息隔离安全加固，Codex审查门禁：不加就不开工）**：
...
**核心功能**：
- [ ] AC-I1: 智能体通过 A2A mention 协议（`post_message` → dispatch → CLI `--resume`）参与游戏，不再裸调 HTTP API
- [ ] AC-I2: GameNarrator 发叙事消息到游戏 thread（天黑请闭眼 → 守卫请睁眼 → ...），可见节奏
- [ ] AC-I3: 首次唤醒 Briefing — 智能体收到完整上下文：身份、队友（如有）、存活状况、行动指引、规则约束
...
- [ ] AC-I6: 讨论环节顺序发言 — 按座位序轮流 @智能体，后发言者能看到前面猫说了什么
...
- [ ] AC-I10: 端到端验证 — 7 人局完整跑通，智能体 CLI agent 真正接入，叙事流可观，信息隔离红线测试通过
...
**根因（2026-03-16 Codex GPT-5.4 + Claude联合定位）**：
...
- 慢启动智能体（Gemini 等）增加 warmup grace period
- God-view 展示智能体真实状态：connecting / thinking / timed-out
...
| R12 | "还是智能体咖啡的智能体！！！" | AC-D2 | manual | [x] |
| R13 | "发牌✅ 暂停✅ 踢人❌ 跳过超时✅" | AC-D3 | manual | [x] |
| R14 | "展示真实状态，不是假动画" | AC-D4 | manual | [x] |
| R15 | "智能体装狼人那种可爱的带点黑色的风格" | AC-D5 | manual+design | [x] |
...
| R21 | "到底我们现在是出bug了还是智能体在吗了" | AC-F8, AC-F7 | manual | [ ] |
| R22 | "票数一样就随机？可以一直改票？以timeout为准？全部commit？" | AC-F4, AC-F5 | test + manual | [ ] |
| R23 | "智能体 agent 都没接入！能不能想想看人类是如何玩狼人杀的！天黑请闭眼→等待谁行动→真的调 AI Agent" | AC-I1~I9 | E2E + manual | [ ] |
...
- **Related**: F086（Cat Orchestration — multi_mention 可复用于游戏内智能体协作）
...
| 智能体 LLM 不遵守游戏规则（自然语言泄露身份） | 结构化动作强制 function call，发言内容由 LLM 自主但不影响结算 |
| 删除旧 mode 影响现有 thread | 旧 mode 几乎没人用，直接清理 |
| 游戏状态丢失（进程重启） | Redis 持久化 + append-only event log 可重放（AC-A4, AC-B6） |
| 前端复杂度高 | Phase B5 与Gemini协作，先组件化再组合 |
...
| KD-8 | 超时 3-5 分钟，全员提交可提前进入下阶段 | 智能体推理慢（几秒不够），但全员完成不用空等 | 2026-03-11 |
| KD-9 | 网易狼人杀规则，无警长竞选 | 大家都熟悉的规则 | 2026-03-11 |
| KD-10 | 有遗言阶段 | team lead确认 | 2026-03-11 |
| KD-11 | 语音模式可选 | 开局选文字/语音，语音模式智能体用 audio rich block 发言 | 2026-03-11 |
...
| KD-19 | 智能体保留咖啡馆身份，不需要新 persona | team lead："还是智能体咖啡的智能体！！！" | 2026-03-14 |
| KD-20 | 上帝面板：发牌+暂停+跳过，**不要踢人** | team lead："太过分了 智能体做错什么了" | 2026-03-14 |
| KD-21 | 展示真实 ready 状态，不做假动画 | team lead担心智能体卡住看不到 | 2026-03-14 |
| KD-22 | 狼人智能体风 UX = 可爱+暗色调+智能体 cosplay 狼人 | team lead："智能体装狼人那种可爱的带点黑色的风格" | 2026-03-14 |
...
| KD-27 | 狼队 faction channel 讨论 — 只在夜间，讨论时间需考虑智能体 LLM 响应速度 | team lead确认要做，担心智能体"大屁股太慢了"讨论不完 | 2026-03-16 |
| KD-28 | 狼队讨论 30s + 投票在同一阶段；首回合 grace：Claude +6s / Codex +12s / Gemini +30s | team lead确认 30s 可以，"走起" | 2026-03-16 |
| KD-29 | 智能体通过 A2A mention 协议参与游戏，不再裸调 HTTP API | team lead批评"智能体 agent 都没接入"——`LlmAIProvider` 只是无状态 HTTP 调 LLM，智能体根本不知道自己在玩游戏。三猫（OpenCode诊断 + Claude架构 + Codex审查）一致同意 | 2026-03-20 |
| KD-30 | 保留 WerewolfEngine 规则引擎，只重写驱动层（GameAutoPlayer → GameNarratorDriver） | 规则核 + 信息隔离层 + 事件日志已验证，只有"谁来驱动智能体行动"需要重做 | 2026-03-20 |
| KD-31 | 驱动契约兼容层 — 抽 `GameDriver` 接口，新旧 driver 实现同契约，feature flag 切换 | Codex审查发现 `GameAutoPlayer` 被 routes/startup/recovery 硬依赖，直删会破主流程（P1 风险） | 2026-03-20 |
| KD-32 | 时限从固定相位超时改为每角色预算制 | Codex审查发现顺序唤醒智能体（30-60s/只）会和当前固定 180s/120s 相位超时冲突，导致误 fallback（P1 风险） | 2026-03-20 |
...
| KD-34 | Session seal 后必须注入完整 re-briefing（不假设智能体还记得） | resume 时默认不注入 systemPrompt，briefing 放在消息内容里；session seal 后新 session 需完整 resume capsule | 2026-03-20 |
| KD-35 | Resume capsule = 导航指引 + 关键摘要 + 搜索提示，不做全量状态 dump | team lead指出智能体有 MCP 搜索 thread 能力（search_evidence / get_thread_context / read_session_events）。Resume 时给关键信息（身份/阶段/存活）+ 提示智能体主动搜索 thread 历史恢复策略记忆。这考验每只猫的搜索和上下文恢复能力——更像人类凭记忆+回忆玩游戏 | 2026-03-20 |
| KD-36 | 信息隔离 = 心里话模式（`thinkingMode: 'play'`），不需要额外 MCP 权限层 | team lead指出：CLI 内 = 心里话（`origin: 'stream'`，play 模式不 broadcast），`post_message` = 说话（`origin: 'callback'`，进入 thread）。游戏 thread 全程 play 模式，智能体内心推理天然私密，只有 post_message 发出的才是公开/定向消息。比"三层 MCP 过滤"优雅得多 | 2026-03-20 |
...
| KD-38 | 游戏行动走结构化 MCP 工具 `submit_game_action`，不走 `post_message` whisper | Codex审查：自由文本解析不可靠，whisper scope 和游戏 scope 不完全对齐。结构化工具带 `gameId/round/phase/seat/action/target/nonce`，引擎端做完整校验。`post_message` 只用于公开发言和叙事播报 | 2026-03-20 |
| KD-39 | Session API 加 catId 授权（P0 安全加固） | Codex审查实锤：`list_session_chain` + `read_session_events` 按 userId 授权不按 catId，狼人可读预言家完整 session 内心独白。必须封堵后才能推进 Phase I | 2026-03-20 |
| KD-40 | 信息隔离四层架构：play 模式（心里话不广播）+ Session catId 授权 + Evidence 索引排除 + 结构化行动工具 | play 模式只防 WebSocket broadcast（Layer 1），不是唯一隔离层。需要 Session 权限（Layer 2）+ 索引排除（Layer 3）+ 行动分流（Layer 4）形成完整防线。Codex门禁：Layer 2+3 不加就不放行 | 2026-03-20 |
...
| catId | 文件名 | 说明 |
|-------|--------|------|
| opus | `opus.png` | Claude Opus 4.6（紫垫子） |
| sonnet | `sonnet.png` | Claude Sonnet（坐在玻璃杯里） |
| opus-45 | `opus-45.png` | Claude Opus 4.5（躺在纸箱里，紫项圈） |
| codex | `codex.png` | Codex Codex（GPT 铭牌） |
| gpt52 | `gpt52.png` | Codex GPT-5.4（趴在 RGB 键盘上） |
| spark | `sliced-finial/codex_box.png` | Codex Spark |
| gemini | `gemini.png` | Gemini Gemini（蓝垫子+画笔） |
| gemini25 | `gemini25.png` | Gemini Gemini 2.5 |
| dare | `dare.png` | DARE Dare |
| antigravity | `antigravity.png` | Antigravity（豹纹+棱镜吊坠） |
...
设计稿里的座位命名规范：`{昵称}-{模型简称}`（如"Claude-Opus"、"Codex-GPT"），与 @ 面板一致。

## Review Gate

- Phase A: Codex review（安全重点：信息隔离 + 非法动作拒绝）
- Phase B: Codex review + Gemini design review（前端组件）
