...
# F070: Portable Governance — OfficeClaw方法论的可复制输出
...
OfficeClaw的猫被派遣到外部项目（如 studio-flow）工作时，完全"失忆"——不知道 3001 端口是OfficeClaw的、不知道 Redis production Redis (sacred)、不知道 SOP，甚至把 dev server 起在 3001 上导致OfficeClaw前端 404。

根因不是"某个配置没带过去"，而是**能力注入 ≠ 治理继承**：现有能力 Hub（F041）已经能跨项目同步 MCP 配置，但 Skills、SOP、铁律、文档架构、Backlog 治理方法论——这些OfficeClaw知识工程的核心——完全没有跨项目携带机制。

team lead的愿景：**OfficeClaw不只是一个项目，是共创工作站。猫是team lead的永久团队，无论出征哪个项目，都带着完整的知识工程方法论。不需要打开其他 coding agent，不需要从零教规则。**

## What

在现有能力 Hub 基础上，扩展"治理层同步"能力：当猫首次被派遣到外部项目时，自动 bootstrap OfficeClaw的知识工程骨架，让派遣猫带着完整方法论工作。

### 定位

- OfficeClaw = **方法论中枢**（methodology hub）：SOP/Skills/协作规范/愿景守护的真相源
- 外部项目 = **独立执行面**（independent execution plane）：用OfficeClaw方法论模板，但拥有自己的 BACKLOG/Feature/ADR
- **分区控制模型**：OfficeClaw治理的是"怎么做"（方法论），外部项目治理的是"做什么"（自己的 backlog/feature）
- OfficeClaw不是外部项目的 BACKLOG 真相源——OfficeClaw只输出方法论模板和工作流规范
...
| 层 | 内容 | 形式 |
|----|------|------|
| 硬约束 | 端口保留表（3001=OfficeClaw前端）、Redis production Redis (sacred)、禁止 self-review、身份不可冒充 | managed block in CLAUDE.md/AGENTS.md/GEMINI.md |
...
| Skills + 路由 | office-claw-skills symlink + manifest.yaml | project-level `.claude/skills/` symlink bootstrap |
| 协作规范 | A2A 交接五件套、愿景守护协议、review 流程 | shared-rules.md |
| 任务态上下文 | 当前 feat 的 AC、链接、phase | 派遣 thread 首条消息注入 |

### 不携带（各项目独立 or OfficeClaw私有）

- MEMORY.md 项目细节（OfficeClaw私有上下文）
- OfficeClaw自己的 ROADMAP.md 条目（OfficeClaw自己的功能规划）
- OfficeClaw自己的 Feature 聚合文件（OfficeClaw自己的 spec）
- OfficeClaw自己的 ADR/lessons-learned 条目（但方法论模板会输出）
- SystemPromptBuilder 实现细节

注：外部项目会有**自己独立的** ROADMAP.md / Feature 文件 / ADR，由外部项目的猫独立管理。OfficeClaw输出的是方法论模板（"怎么写"），不是具体条目（"写什么"）。

## Non-goals

- 不做整仓镜像（不把 `docs/` 全部 symlink 过去）
- 不做 BACKLOG 双向同步（外部项目用OfficeClaw方法论但独立管理自己的 backlog）
...
原则：**带方法，不带OfficeClaw私有账本；带这次任务包，不灌整份 spec**。
技术路径：invoke-single-cat 从 thread metadata 提取 → system prompt injection。

~~**2b. Provider-agnostic Hooks 契约**（AC-1 完整版, TD099）~~ ✅ PR #274

- **目标层**：定义一套 provider-agnostic 的"OfficeClaw标准 hooks 契约"（commit guard、quality gate pre-check）
...
> **一句话**：让OfficeClaw不只"知道同步过没有"，还"知道这次出征带回了什么"。
...
- 选一个真实外部项目，刻意按OfficeClaw SOP 跑完整闭环
...
| 类别 | 优先级 |
|------|--------|
| OfficeClaw安全铁律（端口/Redis/身份） | 不可覆盖 |
| OfficeClaw协作规范（A2A/review/愿景守护） | 不可覆盖 |
| OfficeClaw SOP 工作流 | 安全/协作底线不可替换，执行流程可由外部项目映射/裁剪 |
...
- [x] AC-11: 派遣猫能在外部项目按OfficeClaw feat/backlog/SOP 跑完整闭环 — **Phase 2a ✅ (任务包注入), Phase 3b 待验证**
...
- [x] AC-17: 外部项目执行结果可回流OfficeClaw追踪（派遣任务状态在 Mission Hub 可见，不需要去外部项目找）— **Phase 3a ✅ + Phase 3c ✅**
...
| Gap | Issue | Fix | PR |
|-----|-------|-----|-----|
| governance-pack.ts leaks internal ports (3001/6399) to open-source | office-claw#97 | Source keeps internal defaults; sync-to-opensource.sh transforms port lines to public defaults (Frontend=3003, API=3004). Redis ports stay as-is (6399/6398) — open-source uses same ports. Version bumped to 1.3.0. | #532 |
| Preflight gate blocks new projects without guidance | office-claw#123 | `PreflightResult` extended with `needsBootstrap`/`needsConfirmation`/`bootstrapCommand`; invoke-single-cat emits actionable instructions | #532 |
| setup.sh missing skills sync step | office-claw#21 | Added Step 5/6 to setup.sh: links office-claw-skills/* to ~/.{claude,codex,gemini}/skills (ADR-009 pattern) | #532 |
| Governance blocked shows raw text, no actionable UX | office-claw#154 | Structured `governance_blocked` event + `GovernanceBlockedCard` with one-click bootstrap+retry button. All 4 routeExecution consumer paths capture `errorCode`. Inspired by community PR office-claw#154. | #602 |
...
| methodology hub 留OfficeClaw（分区控制模型） | 避免真相源分裂 | gpt52 建议 |
| 复用 capability-orchestrator | 不新建并行系统 | codex 硬约束 |
| TD099 并入 Phase A | hook 归一化是闭环关键 | codex 建议 |
| 分区控制模型 | OfficeClaw管"怎么做"（方法论），外部项目管"做什么"（自己的 backlog） | gpt52 P1-1 修复 |
