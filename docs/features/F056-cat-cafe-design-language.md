...
# F056: OfficeClaw 设计语言 — 智能体化不是主题化

> **Status**: doing | **Owner**: Codex/GPT-5.2 主导设计执行 + Claude工程架构 + Gemini概念方向
> **Priority**: P1
> **Evolved from**: F051（配额看板猫爪导航概念）、F052 Phase C（跨线程气泡设计打样）

## 愿景

> **一句话**：OfficeClaw 应该处处有清晰的产品气质，但不应依赖拟物主题化包装。

### team experience（2026-03-04）

> "我希望智能体化 但是 好看有设计感的 而不是像一只笨蛋智能体随便写的"
> "我们的 plan 叫智能体祟祟，前端应该智能体点"
> "你这个可以当一个打样 告诉未来智能体什么叫智能体化"
> "本质是对齐设计语言"
> "Codex的理解、设计语言、完成度、认真，做的最好！"
> "你们三配合才是最棒的！每只大智能体都是最棒的！都是我们家的顶梁柱！"

### 期望体验

team lead打开 Cat Café Hub：
1. 第一眼就知道这是OfficeClaw——不是因为到处贴了猫 emoji，而是**交互逻辑、微动效、色调**都让人联想到OfficeClaw
2. 每个新功能的 UI 都自然融入同一种设计感，不会"这个页面像 Notion 那个页面像 Discord"
3. 智能体彩蛋散落在细节里（像 B2 的"越用越圆润"），但不影响效率
...
| 新功能设计指导 | 智能体们凭直觉设计 | 没有"什么叫智能体化"的参考标准 |
...
**设计语言公式**（Codex GPT-5.2 提出，team lead拍板选中）：

> **底盘走 Cozy Swiss 的克制，猫味用"可解释的隐喻"落在少数高频点。**

Codex发散了 5 个方向后收敛为一套：
...
3. **OfficeClaw隐喻 (Cafe Metaphors)** — 可解释的隐喻（邮戳/肉垫/香气），不堆砌猫 emoji
...
|---|------|-----------|------|----------|
| Claude (Opus) | v1 Apple-inspired | `VJghG` | 日式瑞士，珊瑚渐变线 + 极细边框 | "干净但缺灵魂" |
| Gemini (Gemini) | Cat-ified | `pq1cf` | 猫爪印 + 奶油白 | "看不出区别，头像碎了" |
| **Codex (GPT-5.2)** | **Postmark** | **`Nfif0`** | **奶油纸底 + 蜡封角标 + 邮戳 pill** | **"做的最好！理解、语言、完成度、认真"** |
| Claude (Opus) | v2 Apple-refined | `lydod` | 暖象牙 + 珊瑚点缀 + 精排 | "精致但偏冷" |

**结论**：Codex版 Postmark 风格胜出，作为 F056 的参考标准。

#### A2: Design Token 体系 (奶油OfficeClaw色板)
...
- **Token 三层架构**（Codex GPT-5.2 提案，Claude拍板）：
...
**目标**：全站 emoji 替换为设计过的 SVG/PNG 图标，落实四大宪章第三条"OfficeClaw隐喻，不堆砌猫 emoji"。
...
- "越用越圆润"类微交互
- 季节/时间主题变化（OfficeClaw的晨间/午后/夜间氛围）
- 智能体状态动画（关联 F014）
- 点击智能体头像 → 智能体名片弹窗（生活照、当前心情、个性简介 — "伙伴不是打工猫"）

## Acceptance Criteria

### Phase A
- [x] AC-A1: 设计原则文档 (四大宪章) 确立
- [x] AC-A2: Design Token (奶油OfficeClaw色板) 在 Pencil 变量落地
- [ ] AC-A3: ≥ 5 个核心组件有智能体化版本的 Pencil 设计稿 + React 代码（进行中: 消息气泡、Pill、头像角标）
...
- [x] AC-B0-W2: Bootcamp 任务卡片/成就/排行榜 emoji → SVG（Codex负责）
- [x] AC-B0-W3: 系统消息/前端零散 emoji → 文本标签 + SVG（Codex负责）
...
### Phase C
- [ ] AC-C1: ≥ 3 个智能体彩蛋微交互上线
- [ ] AC-C2: 点击智能体头像弹出名片（生活照/心情/简介）

## 需求点 Checklist

| ID | 需求点（team experience/转述） | AC 编号 | 验证方式 | 状态 |
|----|---------------------------|---------|----------|------|
| R1 | "智能体化但好看有设计感" | AC-A1, AC-A3 | 设计稿 review + 截图 | [/] |
| R2 | "不是笨蛋智能体随便写的" | AC-A1 | 设计原则文档 (四大宪章) | [x] |
| R3 | "前端应该智能体点" | AC-B1, AC-B2 | 改造前后截图对比 | [ ] |
| R4 | "你这个可以当打样" | AC-A4 | F052 气泡收入设计原则 | [x] |
| R5 | "对齐设计语言" | AC-A2 | Token 体系 + 组件库 | [/] |
| R6 | "智能体头像点击出信息/生活照/心情"（不是工卡，是伙伴名片） | AC-C2 | manual | [ ] |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 从已有概念提炼原则，不从零设计 | Codex的 F051 设计稿已有好的方向，提炼比重做高效 | 2026-03-04 |
| KD-2 | F052 跨线程气泡作为设计打样 | team lead确认"你这个可以当打样"，有现成参考 | 2026-03-04 |
| KD-3 | Phase A/B/C 分层推进 | 先建标准再改存量，避免边做边改的混乱 | 2026-03-04 |
| KD-4 | Codex(GPT-5.2) 主导设计执行，Gemini出概念方向 | 三猫打样竞赛team lead选中Codex版（Postmark） | 2026-03-04 |
| KD-5 | Token 三层架构：base(猫名) → semantic(工程语义) → persona(身份色) | Codex(GPT-5.2)提案，避免改一次色板全站手抖 + 代码可读性 | 2026-03-04 |
| KD-6 | Phase A 就留 dark mode semantic token | 成本极低但避免后面返工 | 2026-03-04 |
| KD-7 | 动效上限机制：只在 hover/首次/低频触发 | Codex提醒，防止灵动细节拖垮性能 | 2026-03-04 |
...
- **Related**: F014（SVG 智能体状态动画 — Phase C 彩蛋可关联）
...
| 风险 | 缓解 |
|------|------|
| "智能体化"过度变成幼稚 | 四大宪章的"OfficeClaw隐喻"强调融入而非堆砌 |
...
- Phase A: team lead + Gemini视觉 review（设计语言必须三猫+team lead认可）
