...
# F098: Callback Message UX — 智能体传话可视化

> **Status**: done | **Completed**: 2026-03-12 | **Owner**: Claude | **Priority**: P1

## Why

team experience（2026-03-11 16:50，F097 收尾时发现）：

> "你们智能体之间传递消息，有好几个 MCP 传递的消息，有可能你得想想要怎么样的去展现。假设你是 at Codex，你这里是不是得标明Claude to Codex或者Claude箭头Codex？然后如果是 multi mention，你这里也需要标明。然后我们曾经做的悄悄话的功能，那里就会标明team lead跟什么智能体说，你是不是也得那样子去优化一下？"

> "你看你的证据地方的字，我是看不见的。还有，你不觉得它超级突兀吗？跟你的其他的东西是不是一个设计感？"

### 核心痛点

1. **方向不明** — 智能体通过 MCP（post_message / cross_post / multi_mention）传话时，team lead看不到"谁对谁说"。stream 消息知道是谁说的（有 catId），但 callback 消息不标明目标受众
...
4. **Whisper 不对称** — team lead的悄悄话有 "悄悄话 → [猫名]" 方向标注，但智能体的悄悄话只显示 "悄悄话"，没有方向

### 现状分析

| 消息类型 | origin | 方向标注 | 视觉风格 | 问题 |
|---------|--------|---------|---------|------|
| 智能体 stream 输出 | `stream` | 无（catId 足够） | CLI 块 tinted-dark ✅ | — |
| 智能体 callback（post_message） | `callback` | ❌ 无 | 普通气泡 ❌ | 不知道对谁说 |
| 智能体跨 thread 转发 | `callback` + `crossPost` | 📮 badge 有来源 | 普通气泡 ❌ | 只有来源没有方向 |
| multi_mention 结果 | `connector` | ❌ 无 | connector 样式 | 不知道谁被 @ 了 |
| team lead悄悄话 | — | ✅ "悄悄话 → [猫名]" | amber badge ✅ | — |
| 智能体悄悄话 | — | ❌ 只有 "悄悄话" | amber badge | 缺方向 |
...
- **whisper（智能体）** → `悄悄话 → @猫名` （复用 whisperTo 字段，和team lead一致）
...
智能体的悄悄话 badge 从 "悄悄话" 改为 "悄悄话 → @猫名"，和team lead悄悄话一致。

**A4: A2A 内部讨论颜色优化**

当前问题（team lead 17:28 截图）：
- A2A 折叠展开后，内部消息用品种色 `secondary` 做气泡背景（如Codex `#C8E6C9` 浅绿）
...
> "假设你们正在智能体调用这个阶段，我的消息在我们的 channel 里面排队嘛。但是我在前端看到的我的消息展现的位置是在我发的那一刻，但其实不是在你们收到的那一刻。这样子就会给别人一种误解，以后在回顾这整个 thread 的时候，就会分不清楚这些消息到底是什么时候被你们收到了。"

当前问题：
- team lead在智能体调用期间发的消息进入 channel 排队
- 前端展示位置是 **发送时刻**（`timestamp`）
- 实际被智能体处理的时刻可能延后很多（排队等待）
- 回顾 thread 时，消息看起来像是在智能体输出中间插入的，但实际智能体当时还没看到

修复方向（终态设计）：
- 方案 A：消息卡片增加"智能体收到时间"标注（如 `发送 19:05 · 收到 19:12`）
- 方案 B：消息在 thread 中按**实际被处理/收到时刻**排序，而非发送时刻
- 方案 C：在智能体输出流中插入"收到team lead消息"的系统提示，标明延迟
...
- [x] AC-A4: 智能体 whisper badge 显示 "悄悄话 → @猫名"（和team lead whisper 一致）✅
...
### Phase D（消息流位置正确性）
- [x] AC-D1: team lead在智能体调用期间发的消息，回顾时能区分"发送时刻"和"被收到时刻" ✅
...
| R3 | cross_post 方向标注 | Claude分析 | A | AC-A3 |
| R4 | 智能体 whisper 方向对齐team lead whisper | team lead 16:50 | A | AC-A4 |
| R5 | callback 消息视觉统一（不突兀） | team lead 16:50 | A | AC-A5 |
| R6 | Evidence Panel 文字可读 | team lead 16:50 截图 | B | AC-B1 |
| R7 | connector 消息视觉统一 | Claude分析 | B | AC-B2 |
| R8 | 后端 targetCats 元数据 | Claude分析（优化） | C | AC-C1 |
...
| KD-2 | 智能体 whisper 方向标注和team lead whisper 统一 | 一致性，用户心智模型统一 | 2026-03-11 |
