...
# F063: Hub Workspace Explorer — team lead不用打开 IDE 也可以和智能体们优雅协作

> **Status**: done | **Owner**: Claude (Opus 4.6, Leader)
> **Created**: 2026-03-05
> **Completed**: 2026-03-09

## Why

team lead和智能体是**共创伙伴**，但目前协作时team lead被挡在 IDE 门外：

1. 智能体说"看 `codex-event-transform.ts:172`"→ team lead要切 WebStorm、搜文件、找行号、读不是自己写的代码
2. 智能体改了 spec/提示词模板 → team lead要去 IDE 翻文件才能看到内容
3. 遇到反复出现的系统问题需要team lead协助梳理时 → "所有和提示词注入有关的代码在哪？"要智能体回答或自己搜关键词
4. 审计日志/session 事件目前只能在 VSCode 里看，team lead帮忙定位问题需要在 IDE 和 Hub 之间反复切换

**核心判断**：Claude.ai 的 Project Context + Artifacts 能力证明了"在对话旁边直接操作文件和预览"是可行的。以前做这个要人类开发一个月，现在智能体一天就能做——**没有理由先做临时方案再做正式方案**（team experience："绕路了"）。
...
1. **文件树浏览**
   - 当前智能体所在仓库的目录树（如 `office-claw/`、`dare-framework/`）
   - 展开/折叠目录，点击查看文件内容
...
4. **智能体联动**
   - 智能体提到文件路径/行号时 → Hub 自动识别 → 点击跳转到文件内容面板
   - 智能体发 `diff` rich block → 点击可在文件面板中查看完整文件上下文
   - team lead在文件面板中选中代码 → 可直接引用到对话中问智能体
...
1. **HTML/JSX 预览**
   - 智能体输出的 React/HTML 组件 → 在 Hub 内 iframe sandbox 渲染
...
| Worktree 指示器 | 文件面板顶部醒目标签：`🌿 feat/f060` + branch + short sha | Codex(安全)+Gemini(UX) |
| 文件树 | 极简风格，悬浮显示操作按钮，类型图标区分 | Gemini |
| 编辑器 | **CodeMirror 6**（轻量、可扩展、语法高亮+行号） | Gemini提议 |
| 只读/编辑切换 | 默认只读🔒，点击切换编辑🔓（签发 edit_session_token） | Gemini(UX)+Codex(安全) |
| 文件路径联动 | 聊天中 `file:line` 格式自动变为可点击链接 → 右侧跳转高亮 | Gemini |
| 正在编辑指示 | 文件图标旁显示 🐾（team lead）或智能体头像 | Gemini |
| 代码引用 | team lead在编辑器选中代码 → 引用到对话输入框 | spec 原始需求 |
| 文件头信息 | `branch + worktree + last_commit_short_sha` | Codex |
...
- [x] AC-8: 图片文件可直接预览
- [x] AC-9: team lead可在 Hub 内编辑文件，智能体可直接 commit 编辑结果
- [x] AC-10: 文件系统感知 worktree（显示智能体当前 worktree 的文件，而非只有 main）
- [x] AC-11: 顶栏有切换按钮，点击后聊天窗口缩小 + 右侧文件面板展开
- [x] AC-12: 搜索栏支持文件名搜索模式（输入文件名/路径片段 → 快速定位 + 显示相对路径 → 点击导航）
- [x] AC-13: 智能体消息中的文件路径点击后自动切换到 workspace 面板并打开该文件（当前 AC-4 的完整体验闭环）
...
| R3 | "智能体提到了个文件，此时我翻半天，还得找行号" | AC-4 | manual: 点击文件路径跳转 | [x] |
...
| R8 | "这个是我们非常重要的一环体验？如何 ux 如何布局？" | AC-6, AC-11 | visual: Gemini review 布局 | [x] |
| R9 | "聊天窗口变小 文件系统右边代替状态栏出来 五五开" | AC-6, AC-11 | manual: 顶栏按钮切换分栏 | [x] |
| R10 | "如果是可以编辑的话 那有什么我帮你们编辑 复制进来" | AC-9 | manual: team lead编辑+智能体 commit | [x] |
...
| 方案选择 | A 智能体主动发 / B 侧边栏 / C 完整 Project | **直接做 B/C，不做临时方案 A** | team lead (2026-03-05) |
...
| 文件编辑能力 | 只读 / 可编辑 | **可编辑** — team lead帮忙编辑后智能体可直接 commit | team lead (2026-03-05) |
| Worktree 感知 | 忽略 / 感知 | **必须感知 worktree** — 智能体可能在不同 worktree 工作，文件系统需显示对应 worktree 的文件 | team lead (2026-03-05) |
| 参考实现 | 自研 / 参考现有 | **参考 Claude.ai Project + Codex 布局**，取其精华 | team lead (2026-03-05) |
| UI 设计语言 | 通用 / 智能体化 | **对齐 F056 Cat Café 设计语言（智能体化不是猫化）** | team lead (2026-03-05) |
| 设计稿工具 | Figma / Pencil | **Pencil MCP**（用 `pencil-design` skill） | team lead (2026-03-05) |
| 设计稿协作 | 单猫 / 多猫 | **Gemini出灵感（不画），GPT-5.2 可协助画设计稿，Claude用 Pencil 落地** | team lead (2026-03-05) |

## Dependencies

- **Related**: F060（图片渲染能力）
- **Related**: F058（运行时状态展示）
- **Related**: F056（设计语言——UI 智能体化风格必须对齐）
- **Evolves to**: F082（Git Health Panel — repo 状态可视化，从 workspace 基础设施衍生）
- **UX Design**: Gemini出灵感 + GPT-5.2 协助画设计稿 + Claude用 Pencil MCP 落地

## Design Workflow（team lead指定）

实施前的设计稿流程：

1. **灵感**：Gemini/Gemini提供 UX 灵感和方向建议（**不让他画**，幻觉多）
2. **设计稿**：Claude用 **Pencil MCP**（`pencil-design` skill）画设计稿；如需协助可 @gpt52 一起画
3. **设计语言**：所有 UI 元素对齐 **F056 Cat Café 设计语言**（智能体化不是猫化）
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| 文件系统 API 路径遍历漏洞 | 安全隐患 | 白名单 + 路径规范化 + Codex安全 review |
...
- **Reviewer**: 跨 family（Codex关注安全，Gemini关注 UX）
- **Cloud review**: 合入前必须

## Phase 1 UI 改进需求（team lead反馈 2026-03-05）

team lead评价 Phase 1 UI："有点丑不够智能体，感觉没有设计感"。以下是具体问题和改进方向。
...
| U4 | 没有 Cat Café 设计语言 | 通用灰/蓝配色，和 Hub 其他面板风格不统一 | F056 要求：智能体化不是猫化，温暖而专业 |
| U5 | worktree 指示器太小 | `text-[10px]` 绿色 badge，几乎看不到 | 应该醒目：分支名 + 短 SHA + 状态色 |
| U6 | 空状态不友好 | "加载中..." 纯文字 | 应有骨架屏 / 智能体插图 / 引导提示 |
...
| P2B-9 | **BUG**: 引用到聊天不带 worktree 信息 — 格式改为 `` `path` (🌿 branch) ``，让智能体知道引用的是哪个 worktree | AC-15 | **done** |
...
### Phase 2C-fix: 愿景守护修复（2026-03-06 Codex+GPT-5.4 审查）
...
team lead需求：智能体帮外部项目（如 `studio-flow`）开发时，team lead想在 Hub 里看到那个项目的文件，但不想破坏安全隔离。
...
| P3-6 | 持久化策略：写入配置文件（不依赖环境变量重启） | **done** (.office-claw/linked-roots.json) |
...
| Bug | 描述 | 根因 | 状态 |
|-----|------|------|------|
| B1 | 切换 Hub project 后 Workspace 仍显示 office-claw 文件 | 后端 `listWorktrees()` 用 `process.cwd()` 固定指向 office-claw，前端无 project context | **PR #266 已修（后端+hook）** |
...
**问题**：Workspace API 硬编码 `process.cwd()` 作为 git repo root，导致切换 Hub project 后文件树仍显示 office-claw。
