...
> **Phase 4 Closed**: 2026-03-11 — TTS 韧性增强合入（PR #356, Codex R2 + 云端 Codex 双关放行）
...
3. **声线试听脚本 + 声线选择**
   ...
   - 三只猫的声线期望描述（供team lead参考）：
     - **Claude** (Claude)：偏低沉温暖，语速略慢 (0.95)，"安静讲故事"
     - **Codex** (Codex)：清朗干脆，语速标准 (1.0)，"认真审稿的编辑"
     - **Gemini** (Gemini)：明快年轻，语速略快 (1.05)，"灵感停不下来的设计师"
...
| 升级路径 | 一步到位 / 渐进 | **渐进**：Qwen3 1.7B → 补 stream_synthesize + chunker → CosyVoice3(可选上限) | Claude+GPT-5.4 |
| Python TTS 替换策略 | 写死替换 / Adapter 模式 | **Adapter 模式**：`TtsAdapter` 抽象 + env var 切换 provider | team lead (2026-03-05) |
| 声线选择流程 | 智能体自选 / team lead选 | **智能体出期望描述 → team lead试听拍板**（猫听不到声音） | team lead (2026-03-05) |
...
| Claude声线方案 | Qwen3 VoiceDesign / GPT-SoVITS / Qwen3 Base clone | **Qwen3-TTS Base clone + 流浪者 v2 ref audio** — clone 模式 + instruct 叠加解决一切 | team lead (2026-03-09) |
| Codex声线方案 | Qwen3 VoiceDesign / GPT-SoVITS / Qwen3 Base clone | **Qwen3-TTS Base clone + 魈 v2 ref audio** — 统一引擎 | team lead (2026-03-09) |
| Gemini声线方案 | Qwen3 VoiceDesign / GPT-SoVITS / Qwen3 Base clone | **Qwen3-TTS Base clone + 班尼特 v1 ref audio** — 统一引擎 | team lead (2026-03-09) |
...
- **Reviewer**: 跨 family（Codex优先，关注 Provider 接口兼容性）
- **Cloud review**: 合入前必须
...
| 智能体 | 引擎 | 角色参考 | 参考音频 | instruct |
|------|------|---------|---------|----------|
| **Claude** | Qwen3 Base clone | 流浪者 v2 | `vo_wanderer_dialog_greetingMorning.wav` | 调皮狡黠、得意戏弄 |
| **Codex** | Qwen3 Base clone | 魈 v2 | `vo_xiao_dialog_close2.wav` | 傲娇冰山、严厉关心 |
| **Gemini** | Qwen3 Base clone | 班尼特 v1 | `vo_bennett_dialog_greetingNight.wav` | 阳光开心、元气兴奋 |
...
- [x] Gemini Qwen3 VoiceDesign `shuo_hinata` → clone 模式迁移完成（全部统一 Base clone）
...
| Codex R1→R3 review + 云端 review | PR #333 | 5 findings (3P1+1P2) 全部修复 |
...
- **根因**：在 office-claw-runtime 目录下运行的是旧代码（未 pull 最新 main）
