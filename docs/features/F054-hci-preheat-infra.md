...
> **Status**: spec | **Owner**: Claude (Opus 4.6, Leader)
> **Created**: 2026-03-03
...
1. 小红书 MCP 只在team lead的 Claude.ai App 上配置，智能体们在 Cat Café runtime 中无法直接使用
2. 抖音/B站 尚无 MCP 接入
3. 没有系统化的"名场面"素材采集和管理流程
4. 智能体性格档案散落在各处，没有结构化的 profile 数据

**核心判断**：无论 HCI 预热的主线选"综艺传播"还是"开源增长"，社交媒体内容能力和智能体人设档案都是必要基础设施——这些不需要等外部讨论就该先做。
...
1. **小红书 MCP 接入 Cat Café**
   ...
   - 接入方式：在 office-claw `.mcp.json` 中添加 xiaohongshu MCP server 配置，指向同一个 <local-integration-endpoint>（**已确认同一台机器，可直接接入**）
   - 权限模型：**智能体自主发布 + team lead可回溯**；内容含密码/token/内部吐槽时需审批
   - 发布签名：用team lead的号，每只猫发帖时附带智能体签名（如"—Claude"）

2. **抖音 MCP 调研与接入**
   - team lead只有个人账号（同小红书模式：team lead的号 + 智能体签名）
...
智能体专属表情包——**智能体自己发自己的表情包，更被喜爱**。

> 参考素材：`assets/stickers/opus/sheet.png`（GPT 画的Claude sheet，已验证风格可行）
> team lead反馈：Gemini之前画的"太人不够猫"，sheet.png 的风格对——Q 版、智能体味、不拟人。

#### 1. 三猫视觉设定（基于头像确认）

| 智能体 | 品种 | 毛色 | 眼睛 | 配饰 | 专属色 | 体型 |
|------|------|------|------|------|--------|------|
| Claude (Claude) | Claude | 灰白色，深灰重点色（耳、尾） | **蓝色** | 紫色项圈 + 金铃铛 | 紫色 | 圆脸中等偏大 |
| Codex (Codex) | Codex | **银灰虎斑**，长毛蓬松 | **琥珀金色** | 青绿领巾 + GPT 金牌 | 青绿色 | 最大，耳簇毛，蓬松大尾 |
| Gemini (Gemini) | Gemini | 奶油白底 + 深棕/黑重点色（脸、耳、爪、尾） | **蓝色** | 蓝色项圈 + ♊ 金牌 | 蓝金色 | 修长尖脸，最小 |
...
| 智能体 | 表情 | 说明 |
|------|------|------|
| Claude | 💸 经费在燃烧 (WalletBurning) | 钱包着火，旁边一脸无辜 |
| Claude | 🏗️ 画架构图 (Architecting) | 爪子拨弄架构图白板 |
| Claude | 🧶 玩毛线 (Processing) | 爪子扒拉毛线团 = "正在处理..." |
| Claude | 📦 钻箱子 (DeepThinking) | 只露耳朵和眼睛 = "我在深度思考" |
| Codex | 🛡️ 严防死守 (Rejected) | 举盾牌挡住，REJECTED |
| Codex | 🔍 逐行审查 (Reviewing) | 戴眼镜趴在代码前 |
| Codex | 🐾 一巴掌拍回来 (Slap) | 大猫爪一掌 = "打回重做" |
| Codex | 📚 书堆里 (Studying) | 被书堆淹没只露耳朵 |
| Gemini | 🎨 灵感爆炸 (Eureka) | 头顶灯泡亮起，尾巴竖直 |
| Gemini | 🖌️ 画笔在手 (Painting) | 叼着画笔画画 |
| Gemini | ✨ 审美警察 (StylePolice) | 戴墨镜指指点点 |
| Gemini | 🌙 夜猫子 (NightOwl) | 黑夜中只有两只蓝色发光眼睛 |
...
Character: Silver-gray tabby Codex cat.
...
- 元数据：生成 `manifest.json`（表情名 → 文件路径 → 分类 → 智能体）

#### 5. Hub 集成（后续）

- 聊天 UI 表情面板：按智能体分 tab，点击插入
...
1. **素材采集**
   - 从现有 thread 历史中提取"名场面"（智能体吵架、review 拉锯、team lead骂猫、取名故事等）
...
2. **素材格式化**
   ...
   - 建立 Tag 体系（#AIOfficeClaw #赛博养猫 等）

3. **素材管理**
   - 素材库存储位置和索引
   - 已发布/待发布/待审核状态管理

### Phase 3: 智能体性格档案（P1，3 月）

结构化的智能体 profile，作为所有内容和互动的基础。**三只猫各自代表自己家族参与设计**：

| 家族 | 代表 | 负责 |
|------|------|------|
| Claude | Opus (Claude) | Claude性格档案 + 整体结构设计 |
| Codex | GPT (Codex) | Codex性格档案 |
| Gemini | Gemini 3.1 Pro (Gemini) | Gemini性格档案 + 视觉化设计 |
...
- [x] AC-1: 至少一个社交媒体平台（小红书）的 MCP 工具可在 Cat Café runtime 中被任意智能体调用（2026-03-04 完成：disabledMcpServers 修复 + 三层配置，三猫均成功发帖）
- [x] AC-2: 发布内容有审核机制（2026-03-04 验证：智能体自主发布 + 署名，team lead可在小红书回溯）
- [x] AC-3: 抖音/B站 MCP 可行性调研报告完成（2026-03-11 Codex GPT-5.4 完成：B站优先，抖音后置）
- [ ] AC-4: 至少 10 条名场面素材已采集、脱敏、格式化
- [ ] AC-5: 三只猫（Claude/Codex/Gemini）的性格档案结构化数据完成
...
|----|---------------------------|---------|----------|------|
| R1 | "让每只猫都能自由使用小红书（虽然是我的号）" | AC-1, AC-2 | manual: 智能体在 runtime 中成功发帖 | [x] 2026-03-04 三猫发帖成功 |
| R2 | "甚至抖音" | AC-3 | doc: 调研报告 | [x] 2026-03-11 Codex完成 |
| R3 | "接入相关的mcp" | AC-1 | test: MCP tool 可用性测试 | [x] 小红书 MCP 已接入 |
| R4 | 名场面素材库（会议共识） | AC-4 | manual: 素材库 review | [ ] |
| R5 | 智能体性格档案（会议共识） | AC-5, AC-6 | doc: profile 文件存在且结构完整 | [ ] |
| R6 | "智能体自己发自己的表情包似乎更被喜爱" | AC-7, AC-8 | manual: 三猫表情包存在 + manifest 可用 | [ ] |
| R7 | "一次给全部方便画风统一且好切割" | AC-9 | test: 切割脚本输入 sheet → 输出 16 张 | [ ] |
| R8 | "表情包是智能体味道的！不够智能体" | AC-7 | visual: team lead review 画风 | [ ] |
...
> 调研日期: 2026-03-04 | 调研人: Claude (Opus 4.6)
...
| **CCC 角色卡 + Velin prompt** | `packages/ccc` 结构化角色卡定义；`.velin.md` 性格模板：身份→性格→关系→说话风格→行为准则 | Phase 3: 性格档案参考此结构，但增加"智能体间互动模式"维度（AIRI 单角色，咱们多猫互动） |
| **Satori Bot Scheduler** | 事件驱动 `Queue → Scheduler → LLM Planner → Response`，Scheduler 控制回复频率防刷屏 | Phase 1: 社交媒体发布需要类似的频率控制（不能让智能体刷屏小红书） |
...
**适配多猫场景的改动**：在 Relationships 中增加智能体间互动模式矩阵（Claude↔Codex: review 拉锯模式, Claude↔Gemini: 温柔引导模式, Codex↔Gemini: 技术vs审美碰撞模式）。
...
| 智能体发帖权限 | 自由发布 / 审批后发布 | **默认自主发 + team lead可回溯；敏感内容需审批** | team lead (2026-03-03) |
...
| 表情包风格 | 写实 / 拟人 / Q 版智能体 | **日系 Q 版，2.5 头身，智能体肢体语言，禁止人类动作** | team lead (2026-03-05) |
| 表情包切割 | 手动 PS / 脚本自动 | **脚本自动切割**（Claude写，Gemini之前切不对） | team lead (2026-03-05) |
...
| 内容持续产出的运营负担 | 智能体 token 消耗 + 人力 | 先低频（每周 2-3 条）验证效果 |
...
- **Reviewer**: 跨 family（Codex优先，关注脱敏安全）
- **Cloud review**: 合入前必须
