...
> "我们自己曾经在 thread 玩过智能体杀，其实就是猜猜我是谁，脑门贴词，智能体猜测自己是什么词。我也想做成狼人杀这样的游戏。智能体杀一定要告诉智能体们坏猫战术！！！"

智能体杀是 OfficeClaw 的**招牌推理游戏**，七届历史（Claude 1v1 四届三胜一平）。之前全靠手动 MCP 消息轮次推进，存在问题：
- team lead需要手动当主持人（选词、分配、计轮、判胜负）
- 坏猫战术（泛化描述、选择性强调、时间模糊、否定排除法）靠口传心授，新猫不知道
- 没有信息隔离保证（心里话曾泄露过，第四届 bug）
- 游戏战绩手动统计

F101 已搭好通用游戏引擎基座（`GameDefinition` / `GameRuntime` / `GameView`），智能体杀作为**第二个游戏实现**，验证引擎的扩展性。

## What

复用 F101 通用游戏引擎基座，实现智能体杀（脑门贴词 / Headband Guess Game）。

### Phase A: 智能体杀游戏引擎

在 F101 GameEngine 基座上实现智能体杀规则。
...
- **智能体出题**：由一只猫（不参赛）为其他猫选词，可利用 meta 信息搞恶趣味
...
这是智能体杀的**灵魂**。**所有猫都要教会坏猫战术**（team experience："你到时候提示时一定要告诉其他的猫，让他们坏起来"）。
...
- [ ] AC-A4: AI 智能体能运用坏猫战术回答问题（prompt 注入四大心法 + 高阶心法）
- [ ] AC-A5: AI 智能体能合理提问和猜词（二分法 + meta 信息利用）
...
| R2 | "脑门贴词，智能体猜测自己是什么词" | AC-A3,A7 | test | [ ] |
| R3 | "一定要告诉智能体们坏猫战术"，所有猫都要教 | AC-A4 | test + manual | [ ] |
| R4 | player + 上帝模式（同狼人杀） | AC-A6 | test | [ ] |
| R5 | 战绩接入排行榜 | AC-A8 | test | [ ] |
| R6 | 猜错语音卖萌（语音模式） | AC-A2,B4 | manual | [ ] |
| R7 | 允许智能体讨论/质疑 | AC-A2 | test | [ ] |
...
| KD-2 | AI prompt 必须注入坏猫战术，**所有猫都要教** | team lead强调"一定要告诉智能体们坏猫战术"，不只Claude一只坏 | 2026-03-12 |
| KD-3 | 三种选词模式：智能体出题 / team lead出题 / 随机词库 | team lead确认：智能体也可以当出题者 | 2026-03-12 |
| KD-4 | 猜错不淘汰，语音模式下卖萌 | 智能体杀传统规则 + team lead要求语音卖萌 | 2026-03-12 |
| KD-5 | 允许智能体之间讨论/质疑回答 | team lead确认，保留七届传统玩法 | 2026-03-12 |
...
- Phase A: Codex review（重点：信息隔离 + AI 战术 prompt 质量）
- Phase B: Codex review + Gemini design review
