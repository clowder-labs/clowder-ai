...
# F105: opencode 接入 — OpenCode（开源多模型编码猫）

> **Status**: done | **Owner**: Claude Opus 4.6
...
OfficeClaw 已有 DARE（DARE，L1 CLI）和 Antigravity（Antigravity，CDP 桥）两条外部 agent 接入通道。现在需要接入第三位：**opencode** — 一个开源、provider-agnostic 的 AI coding agent。
...
4. **原生 MCP 支持** — opencode 内建 MCP client，与 OfficeClaw MCP 编排天然兼容
5. **强 TUI/主题生态** — 社区活跃，插件丰富

team lead定性：**OpenCode** —— 名称强调开放、多模型、稳定接入能力，符合该 provider 的定位。

---

## What

通过 L1 CLI Adapter（复用 F050 DARE 模式），将 opencode 作为独立家族（OpenCode）接入 OfficeClaw。
...
| 维度 | DARE/DARE (F050) | Antigravity/Antigravity (F061) | opencode/OpenCode (F105) |
|------|---------------------|------------------------------|------------------------|
...
- **OMOC Sisyphus 编排器**：仅管理OpenCode自己的内部专家子 agent（Oracle/Librarian/Frontend 等），即只编排 opencode 自己的 API 调用
- **OfficeClaw CatOrchestration**：管理跨猫调度（OpenCode ↔ Claude/Codex/Gemini）
- **不允许 Sisyphus 编排其他 OfficeClaw 智能体**
...
| 道具 | 终端屏幕 </> |
| 垫子 | 深翠绿 |
| 句柄 | `@opencode`, `@OpenCode`, `@golden` |
| 角色 | coding, multi-agent-orchestration |
| 特长 | 开源多模型编码 agent，自带 OMOC 多专家编排 + LSP + 主题生态 |
| 注意 | OMOC Sisyphus 只编排自己的子 agent，不编排其他猫；opencode 原生 MCP 和 OfficeClaw MCP 需避免冲突 |
...
- [x] AC-7: `cat-config.json` 注册OpenCode（roster + breed golden-chinchilla + variant opencode-default，6 config tests green）
- [x] AC-8: AgentRouter 注册 `case 'opencode'`，cat-config-loader 解析验证通过

### Phase 2: OMOC 集成 + 高级能力 ✅ COMPLETE
- [x] AC-9: OMOC Sisyphus 编排限制在OpenCode内部子 agent — 5 isolation tests: delegate-task targets are OMOC-internal (oracle/librarian/frontend-engineer), no OfficeClaw handles in events
- [x] AC-10: opencode MCP 与 OfficeClaw MCP 编排不冲突 — 5 namespace tests: no MCP env leakage, no CLI MCP flags, zero tool name overlap, config isolation by process + file boundary
...
4. **MCP 双注入** — opencode 自带 MCP + OfficeClaw MCP 编排可能产生工具冲突
...
- **F061**: Antigravity接入（参考 provider 扩展模式）
