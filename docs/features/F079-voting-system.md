...
**Phase 1 问题**：前端 UX 极差（CLI 单行解析、没有通知智能体、手动 cast/end）。
...
在 `AgentRouter` 或 message processing pipeline 中，对智能体回复做 regex 匹配：
...
## Gap 4: 智能体发起投票（MCP 工具）

### Why

当前只有team lead通过 UI `/vote` 命令发起投票。智能体在协作讨论中需要集体决策时（如"这个 API 用 REST 还是 GraphQL？"），无法自主发起投票，必须请team lead操作。

### Scope

新增 MCP 工具 `cat_cafe_start_vote`，让智能体通过 MCP 调用发起投票，复用现有 vote API：
...
**提示词**：
- Skills 中告知智能体有 `cat_cafe_start_vote` 工具
- 使用场景：多猫讨论需要投票决策时

### Acceptance Criteria

- [ ] MCP 工具 `cat_cafe_start_vote` 可被智能体调用
- [ ] 复用现有 vote API（不重复实现）
- [ ] 发起者为 catId，不是 'system'
- [ ] 投票通知正确路由到 voters
- [ ] Skills/提示词更新，智能体知道有这个工具

### 讨论来源

Thread `thread_mmgfvvq1iut03rjs` (2026-03-08 18:13) — team lead提出智能体也该能发起投票
...
5. `[VOTE:xxx]` regex 拦截（不需要智能体学新命令，自然嵌入回复）
...
**现象**：投票结束后，结果卡片出现了两次——一次是系统发的"投票结果" connector 气泡，一次是最后投票的猫（Gemini）的回复里又嵌了一份相同的结果卡片。
...
**复现**：2026-03-08 21:05 投票"我们开源仓叫什么好呢？"，Gemini最后投票后触发。
