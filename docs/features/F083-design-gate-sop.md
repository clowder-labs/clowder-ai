...
team lead发现智能体有时 UX 没确认就直接开写代码，写完才发现不是他想要的。F076 Mission Hub 那次Claude做对了（采访→画图→讨论），但不是每次都这么做。需要把"先确认设计再动手"固化到 SOP 里。
...
- 纯后端 API/数据模型 → collaborative-thinking 智能体讨论
- 架构级变更 → 智能体讨论 + team lead拍板
- Trivial → 跳过，按 SOP 例外路径判断

### Cloud Reviewer Quota Fallback（merge-gate Q4）

云端 reviewer 没配额时的降级策略：同族换个体 / 跨族降级，禁止Gemini（Antigravity Opus 除外），降级后仍须校验 reviewer ≠ 作者。
...
2. 分流判断标准："用户能看到的改动 → 找team lead；看不到的 → 智能体自己搞定；动了骨架 → 两边都过"
