...
# F085 Hyperfocus Brake — 智能体健康小刹车
...
| **5 - UX 增强** | Hub 开关 + TTS 自动播放 + 智能体图片 | 完整感官体验 |
...
- [x] **AC20**: 三猫语音轮流撒娇（Claude→Codex→Gemini，各用自己声线）

### Phase 4 (平台化)

**Gap**: Phase 1-3 的 hook 方案绑在 Claude Code 的 `settings.json` 上，只有Claude能触发提醒。Codex（Codex）和Gemini（Gemini）的 session 完全不覆盖。根因：把平台级能力挂在了 agent 工具链上。

- [x] **AC21**: 后端 API 活跃时长追踪 — 每次 API 请求更新 `lastActivityTs`，5min 间隔检测
- [x] **AC22**: 后端触发判定 — 到阈值推 WebSocket event `brake:trigger` 给前端
- [x] **AC23**: 前端 UI 通知 — 订阅 brake event，弹智能体提醒卡片（含头像 + 撒娇文案）
...
**Gap**: Phase 4 把提醒迁到了前端，但 UX 仍是朴素的：没有语音（之前裁出的 TD108）、没有智能体图片、没有开关。team lead想要：(1) Hub 里能开关 brake，(2) 弹窗时智能体语音自动播放，(3) 弹窗里有智能体图片增加情感。
...
- [x] **AC30**: 智能体图片增强 — brake 弹窗内三猫头像从 36px 放大 + 增加智能体表情/动作图片（撒娇、睡觉、叉腰），提升情感冲击力
...
| 决策 | 结论 | 理由 |
|------|------|------|
| 触发源 | Hook-first + `/loop` 兜底 | Codex Codex 建议，现有 hook 体系成熟 |
| 计时基准 | 活跃工作时长（非 wall clock）| 避免离开吃饭回来误报 |
| Phase 1 交互 | typed check-in only | 终端场景鼠标不稳，可访问性优先 |
| 肉垫点击 | → Phase 2 (Web) | Codex建议 |
| 三猫调用 | orchestrator 生成三段文案 | GPT-5.4 建议，不真拉三模型 |
| 声线顺序 | Claude → Codex → Gemini | 按家族顺序 |
| Phase 4 平台化 | hook → API + 前端 | Phase 1-3 只覆盖 Claude，CodexGemini无保护 |
| Phase 5 TTS | 复用 `useTts` + `AudioBlock` | F066 前端播放基建已就绪，TD108 可直接回收 |
| Phase 5 图片 | 放大头像 + 新增表情动作图 | team lead明确要求"智能体发图片" |
...
- [x] Phase 1: Codex Codex review hook 安全性 (R1-R4 本地 + R1-R2 云端, PR #329)
- [x] Phase 2+3: Codex Codex review LGTM (0 P1/P2, P3 补测试已修)
- [x] Phase 4: Codex Codex 本地 R1-R2 (2P1+1P2 全修) + 云端 R1-R2 (1P1 全修, LGTM)
- [x] Phase 5: Codex Codex 本地 R1 (2P1+1P2 全修) + R2 LGTM (0 P1/P2)
...
| 5 | emergency bypass | Codex Codex | done | state.test.sh |
| 6 | 夜间模式 | Gemini | done | integration.test.sh |
