...
> **Status**: Phase 1-6+A+B+C+D+E+G(8A)+8 done | **Owner**: Claude

## Why

Cat Café 目前只能通过 Web UI 和智能体对话。team lead和未来用户希望在**已有的工作聊天工具**中直接与智能体交互，不用切换窗口。
...
| Phase | 内容 | 状态 | PR |
|-------|------|------|-----|
| **1 (MVP)** | 飞书 + Telegram DM-only 双向对话 | ✅ | [#328](https://github.com/zts212653/office-claw/pull/328) |
| **2** | 多猫身份 + 分角色展示 + 外部 @路由 | ✅ | [#336](https://github.com/zts212653/office-claw/pull/336) |
...
| **4** | 消息编辑模拟流式（placeholder → edits → final） | ✅ | [#350](https://github.com/zts212653/office-claw/pull/350) |
| **C** | 架构归一：命令管道统一 + 跨平台 thread | ✅ | [#353](https://github.com/zts212653/office-claw/pull/353) |
| **D** | `/use` 模糊匹配：feat号 + title关键词 + 列表序号 | ✅ | [#355](https://github.com/zts212653/office-claw/pull/355) |
| **5** | 图片/文件收发（双向） | ✅ | [#362](https://github.com/zts212653/office-claw/pull/362) |
| **6** | 语音消息（STT/TTS） | ✅ | [#362](https://github.com/zts212653/office-claw/pull/362) |
| **E** | 飞书卡片身份标识：所有回复走 interactive card + 猫名头部，消除多猫气泡合并 | ✅ | [#389](https://github.com/zts212653/office-claw/pull/389) |
| **G (8A)** | IM Hub thread：命令隔离（控制面/对话面分离，双绑定）+ Hub thread 可见入口 | 🚧 in-progress | [#570](https://github.com/zts212653/office-claw/pull/570) |
...
| **8** | IM Hub 配置向导 — 平台接入引导 UI（飞书/Telegram/钉钉） | ✅ | [#680](https://github.com/zts212653/office-claw/pull/680) |
| **J1** | file block 全链路 + outbound 投递 + 安全防护（URL 白名单 + path traversal guard + fileName 透传） | ✅ | [#689](https://github.com/zts212653/office-claw/pull/689) |
| **J2** | Pandoc 文档生成服务 + MCP tool + 自动安装（init-office-claw.sh / install.sh） | ✅ | [#693](https://github.com/zts212653/office-claw/pull/693) |
...
**背景**：team lead希望猫能生成 PDF/DOCX/MD 等文档并通过飞书/Telegram 发送给用户。OpenCode已在飞书测试 thread 中验证过文件生成能力。飞书 API 原生支持 `file_type: pdf/doc/xls/ppt/stream`，上传限制 30MB。
...
**J2 已完成（PR #693）**：
- [x] 启动脚本自动安装 pandoc（init-office-claw.sh brew / install.sh apt+dnf）
- [x] PandocService: `execFile('pandoc', ...)` 封装 + 检测缓存
- [x] 降级链 graceful degradation（PDF→DOCX→MD）
- [x] MCP tool `cat_cafe_generate_document` + callback endpoint（已在当前桌面端构建中移除）
...
- **ISSUE-2**: Cloudflare Access 与 webhook 路径冲突 — 临时用 `api.office-claw.com`。详见 [架构归一设计](assets/F088/architecture-unification.md#issue-2-cloudflare-access-与-tunnel-ingress-路径冲突)
