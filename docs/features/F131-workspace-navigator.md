...
# F131: Workspace Navigator — 智能体可编程导航 Workspace 面板

> **Status**: done | **Owner**: OpenCode | **Priority**: P2 | **Completed**: 2026-03-23

## Why

team lead 2026-03-20 语音指示（逐字）：

> "你最好也有自己的 skills，能够让智能体。我跟智能体说，现在我们一起来看一下审，看一下日志。你能帮我去把右边的 workspace 面板打开？当然，这个不只是日志哦，就有点有点多了。可能是你，我们一起看一下怎么样的文档，你也能一起帮我把文档直接打开，就不要我一个个去点。"

> "跟浏览器的 Preview 一样。更通用的是，我用语音或者用文字告诉你，你帮我一起打开这个 workspace 的哪个地方？你要能帮我打开。"

team lead 2026-03-21 进一步明确：

> team lead不会给精确路径。"帮我打开日志""看看 F131 的设计图""打开那个 discussion"——智能体自己能 glob/grep 到路径，自己去传精确路径到 API。对人类来说说全路径太不友好了。

核心痛点：team lead让智能体一起看某个文件/目录时，只能靠自己在 Workspace Explorer 里手动点击层层目录。智能体有 `setWorkspaceOpenFile` / `revealInTree` 等前端能力，但没有对外暴露的 HTTP 端点供智能体调用。browser-preview 的 `auto-open` 已证明这种模式可行且体验好。
...
**参照 F120 browser-preview 的 `auto-open` 模式**，搭建智能体→Hub 的通信管道：

1. **后端 API**：`POST /api/workspace/navigate`
   ```json
   {
     "path": "packages/api/data/logs/api/",
     "worktreeId": "office-claw-runtime",
     "action": "reveal"
   }
   ```
...
### 2. 硬实力层（Agent 能力）— 智能体自己解析路径

智能体收到模糊意图后，**用自身工具（glob/grep/read）找到精确路径**，然后调用基础设施层的 API：

```
team lead: "帮我打开 F131 的设计图"
  ↓
智能体: glob("**/F131*.pen") → 找到精确路径
  ↓
智能体: curl POST /api/workspace/navigate { path: "designs/F131-xxx.pen", action: "open" }
  ↓
Hub: 右面板自动打开并导航到文件
```

这一层不需要后端做任何"智能解析"——智能体本身就是路径解析器。关键是 **Skill 文档要教会智能体怎么做模糊意图匹配**。

### 3. 软实力层（Skill）— 教智能体做意图匹配 + 导航

`office-claw-skills/workspace-navigator/SKILL.md`：

- **触发词识别**：「看看代码」「打开文件」「看日志」「帮我打开」「一起看看」「打开设计图」
- **意图→路径匹配策略**：教智能体根据不同意图类型用不同搜索策略
...
- [x] AC-1: 智能体调用 `POST /api/workspace/navigate` 后，Hub 右面板自动打开 workspace 模式并导航到指定路径 ✅ PR #611
...
- [x] AC-6: 端到端验证——team lead说"帮我打开日志"，智能体能自己找到路径 → 调 API → Hub 右面板自动展示日志目录 ✅ 2026-03-23 runtime E2E（含 PR #678 回归）
...
| 智能体意图匹配不准（找错文件） | Skill 文档提供明确的搜索策略 + 多结果时让team lead确认 |
...
| KD-2 | 日志一键跳转按钮作为 F130 Polish 独立实现，不依赖 F131 | 按钮是 UI 入口，F131 是智能体编程式能力，解耦更灵活 | 2026-03-21 |
| KD-3 | 不分 Phase A/B，单 Phase 三层：基础设施层 + 硬实力层 + 软实力层(Skill) | team lead拍板——模糊路径解析是智能体的 Agent 能力（硬实力），不需要后端做；Skill 教智能体怎么做（软实力）；API/Socket 是管道（基础设施） | 2026-03-21 |
| KD-4 | 智能体传给 API 的路径必须是精确路径，模糊意图解析在 Agent 侧完成 | team lead："我不会告诉你全路径，你自己能 glob 到的" — Agent 本身就是路径解析器，无需后端 LLM | 2026-03-21 |
