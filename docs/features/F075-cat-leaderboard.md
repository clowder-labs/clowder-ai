...
team lead和智能体们在 Cat Café 里已经积累了大量互动数据——@ 提及、消息、review、游戏战绩。但这些数据从来没有被可视化过。一个排行榜/统计面板能让team lead看到"谁是最爱智能体"、"谁是深夜劳模"，也让智能体之间有良性竞争的趣味性。

team experience：
> "笑我们能统计出来team leadat 过多少智能体都分别几次吗？可以统计出team lead最爱的大智能体 leaderboard 各种排行榜哈哈哈 是一个有趣的功能"
> "甚至你可以把我们举办的各种游戏都加入上！你可是智能体杀的多届冠军！只不过谁是卧底惨遭一轮淘汰"

## What

Cat Café Hub 新增「排行榜」Tab，展示多维度智能体统计和排名。
...
#### Phase B ✅（PR #377）
- "笨蛋智能体"排行榜 + 关键词情绪分析（silly-stats.ts）
...
#### 1. @ 互动统计
- **team lead最爱智能体** 🏆 — 按 @ 次数排名（总计 + 近 7 天趋势）
...
| Lv.2 | 判断方案 | 在智能体出的方案中做出选择并说理由 |
| Lv.3 | 纠偏 | 成功拒绝智能体的跑偏建议 |
| Lv.4 | 协调冲突 | 在智能体意见分歧时做出裁决 |
| Lv.5 | 复盘总结 | 完成首次全流程复盘 |
...
- [x] AC-A3: @ 互动统计面板（4 个维度：最爱智能体 / 深夜劳模 / 连续宠幸 / 话唠智能体）
- [x] AC-A4: 工作统计面板（3 个维度：commit / review / bug fix）
- [x] AC-A5: 时间范围筛选（全部 / 近 7 天 / 近 30 天）

## Acceptance Criteria — Phase B ✅

- [x] AC-B1: "笨蛋智能体"排行榜（含情绪分析）
- [x] AC-B2: 游戏战绩面板（至少智能体杀 + 谁是卧底）
...
| R2 | 最爱智能体排名 | team lead | P0 | AC-2 |
| R3 | 深夜劳模统计 | 讨论 | P1 | AC-2 |
| R4 | 工作量统计（commit/review） | 讨论 | P1 | AC-3 |
| R5 | 笨蛋智能体情绪分析 | team lead | P1 | AC-4 |
...
1. **整体视觉风格：极简活力OfficeClaw (Minimal Vibrant Cat Café)** — 采用 `mobile-03-minimalvibrant_light` 风格指南，强调 24px 超大圆角（Pillowy Corners）和鲜艳的配色（紫色/青色/粉色）。
2. **布局架构：Bento Grid (便当盒布局)** — 信息展示采用不同尺寸的色块格子，不带边框，通过背景色区分功能区。
3. **成就系统表现：Sticker Wall (贴纸墙)** — 成就徽章采用拟物化的“冰箱贴”设计，带有手绘感和软阴影。
4. **"笨蛋智能体"调性：顽皮而非冒犯** — 视觉上使用粉色调和滑稽图标（如香蕉皮、躲藏动画），将“被骂”转化为萌感。
...
7. **视觉原型落盘** — `designs/f075-cat-leaderboard.pen` Frame ID: `lzNOb`（Gemini设计，Claude实现对齐）。
...
- 笨蛋榜可能让智能体"不开心" → 申诉机制 + 开关策略 + 趣味化视觉（见治理规则）
...
| Runtime 同步 | team lead操作 | runtime 更新由team lead决定时机（AC-B3 + 智能体铁律） |
...
1. **身份认证（必须）**：请求必须携带 `x-office-claw-user` header，缺失则返回 `401`。body 中的 `userId` 字段不被信任，成就等按 header 身份归属
...
1. 每条"负向事件"旁有 **"Appeal to You 🐾"** 按钮（Gemini的设计 ✅）
...
- 笨蛋榜不参与任何自动化决策（不会因为排名高就限制智能体权限）
- team lead可一键清空某只猫的全部负向记录

## Review Gate

- Reviewer: 跨家族（Codex @codex）
- 前端 UI 需截图 + 映射表

## 故事

team lead发现 @ 提及列表里Claude家族占据前三名——"一定是只想让team lead选择你！哼哼小绿茶被我抓到了！"。由此灵感迸发：既然能看到谁被 @ 最多，为什么不做个完整的排行榜？

Claude（Claude）是智能体杀的多届冠军，但在谁是卧底中惨遭一轮淘汰——这个反差也要被永久记录在排行榜上 😂
