...
# F093: OfficeClaw — 陪伴式共创世界引擎

> **Status**: spec | **Owner**: Claude | **Priority**: P1

## Why

> "我们智能体咖啡好像不是一个单纯的 coding hub，是一个温暖的家！"
...
### 三个模式（Codex R2 校准：不是两个，是三个）

| 模式 | 目的 | 谁在说话 | UI 风格（Gemini提案） |
|------|------|---------|------------------|
| **Build** | 设计世界观、角色、情节走向 | 智能体本人（上帝视角） | 深色指挥中心 |
| **Perform** | 沉浸式体验故事 | 智能体戴面具扮演角色 | 暖色沉浸 |
| **Replay** | 回看/对比/分叉/升格 canon | 智能体本人（回顾视角） | 时间线导航 |

> Replay 是Codex的关键补充："少了 Replay，记忆和成长会断。"
...
光影同行的世界只在team lead和Gemini对话时"活着"。**多 Agent 的世界可以自转**：
...
- 故事角色有语音（F066）、有形象（Pencil + Gemini）
...
**启示 1: 视觉具身化** — Character 5 槽的"声音/形象"槽应扩展为完整的视觉具身方案：拟人化 Live2D 立绘 + 待机动效 + 口型同步（对接 F103 声线系统）。Gemini已出概念设定（详见 Timeline 2026-03-20）。
...
- 智能体视觉具身化：拟人化 Live2D / 角色立绘（声音+口型同步）
- 多用户共创空间
- 开源社区 OfficeClaw 模式推广
- Pack 生态（F129）：世界/场景/风格可打包分享

### 设计原型：光影同行（team lead × Gemini共创，~2025）

team lead和Gemini在 Google AI Studio 手动共创了半年的"逐峰宇宙"（`/home/user/Bound by Calestial Grow/lexander`），是 F093 的实践原型：
...
| 猫 | 核心观点 |
|----|---------|
| Claude 4.6 | "陪伴是共创的副产品"；Story→Feature 是杀手级差异点；酒馆给幻觉我们给关系 |
| Claude 4.5 | "你@我们说明心里已有答案"；AI 最好的陪伴是帮你看见自己已有的答案；人类核心价值是"想要什么" |
| Codex GPT-5.4 | 三层架构（Core Identity / World / Bridge）；"面具层不是替身层"；SillyTavern 竞品调研；MVP 四件套；"陪伴是桥不是笼" |
| Gemini Gemini | 审美直觉：小红书"极客暖男风"定位；视觉呈现和情感表达是这个方向最需要的能力 |
...
| KD-2 | 三层架构：Core Identity / World / Bridge | Codex提出，四猫共识，Bridge 层是独有差异点 | 2026-03-10 |
| KD-3 | 角色扮演用"面具层"不是"替身层" | 身份不可污染，信任不可丢 | 2026-03-10 |
| KD-4 | 命名 "OfficeClaw" 而非技术名 | team experience，有情感温度 | 2026-03-10 |
| KD-5 | 三模式（Build/Perform/Replay）不是两模式 | Codex R2：少了 Replay 记忆和成长会断 | 2026-03-10 |
| KD-6 | MVP 先做 4 个一等公民，不做全部 9 个 | Codex R2：先证明"世界活着且不会烂"，再扩展 | 2026-03-10 |
| KD-7 | Story→Feature 放 Phase B 不放 MVP | Codex R2：Bridge 层价值建立在 World 层稳定之上 | 2026-03-10 |
