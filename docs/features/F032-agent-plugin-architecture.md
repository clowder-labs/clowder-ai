...
> **Status**: done | **Owner**: Claude (Opus 4.5) + 三猫
> **Created**: 2026-02-26
...
1. **身份 = 物种 = 个体**：系统把三个概念糊在一起
   - Family（物种/厂商）：Claude、Codex、Gemini
...
3. **协作规则硬编码**（审计发现 6+ 处）：
   - SOP.md Reviewer 配对表写死"Claude ↔ Codex"
   - CLAUDE.md "Claude找Codex，Codex找Claude"
   - merge-approval-gate skill "没有Codex放行不能合入"
   - 所有 skill 示例只覆盖Claude↔Codex这一对
...
```json
{
  ...
  "roster": {
    "opus-45": {
      ...
      "lead": true,
      "available": true,
      "evaluation": "主架构师，深度思考能力强，但很贵！没配额时别找他"
    },
    "opus-46": {
      "family": "ragdoll",
      "roles": ["architect"],
...
| `available` | boolean | **是否有配额**！false = 不要找他 |
...
interface ReviewerMatchOptions {
  ...
  excludeUnavailable?: boolean;      // default: true — 没配额的不要找！
}
...
- 如果不同 family 都没配额或不可用，允许降级到同 family 的 lead（非自己）
...
**Before (skill)**:
```markdown
**Core principle:** 没有Codex明确放行，不能合入 main。
```
...
- Claude没配额时，Codex可以降级找同 family 的其他Claude分身
- 必须 log 警告让team lead知道降级发生了

**`available` 字段**：team lead可以标记某只猫"没配额"，系统会自动排除。

> **教训**：2026-02 上周 SOP 写死了"Claude ↔ Codex"，Claude没配额了Codex还疯狂找他 review，烧了team lead 40 美刀 extra！
...
| CLAUDE.md L15 | "Claude找Codex，Codex找Claude" | P1 |
| AGENTS.md L129 | "我的代码谁来 review？Claude" | P1 |
| AGENTS.md L461 | "找Codex review"（自我 review bug） | P0 |
| GEMINI.md L13 | "合入前必须经Codex review" | P1 |
| merge-approval-gate | "没有Codex放行不能合入" | P1 |
| requesting-cloud-review | "本地Codex放行后" | P1 |
| 所有 skill 示例 | 只覆盖Claude↔Codex | P2 |
