...
> **Status**: in-progress (Phase A ✅) | **Owner**: 布偶猫 | **Priority**: P1
...
WebSocket 双通道 + 签名认证 + 纯文本收发。**已实现并通过实测验证（2026-03-27）。**
...
| ID | 生命周期 | 用途 |
|----|---------|------|
| `conversationId`（= `params.sessionId`） | 持久不变，同一用户-智能体对话 | **作为 externalChatId 绑定 thread** |
| 顶层 `msg.sessionId` | 每次小艺 app 打开时更换 | 仅日志参考，不可作为绑定键 |
| `taskId`（= `params.id`） | 单条消息（一问一答） | 用于出站回复的 artifact-update 定位 |

> **教训**：最初用 `msg.sessionId` 作绑定键，导致 app 重启后丢失旧 thread。修复后改用 `params.sessionId`（= conversationId），binding 持久稳定。

**已实现**：
- `XiaoyiAdapter`（implements IOutboundAdapter）— 双 WS 内置于 adapter，未独立抽 DualWebSocketManager
- 消息去重（`seenMessages` Map，5 分钟窗口）+ 独立心跳
- HMAC-SHA256 签名生成（Base64）
- Gateway bootstrap 集成 + ConnectorDefinition 注册
- `frontendBaseUrl` 透传到 gateway deps（修复前 URL 回落到 localhost:3003）
...
**遗留项**：
- `[XiaoYi] ← recv` debug 日志仍保留，待完整验证后移除
- AC-A6 前端渠道管理 UI 配置未做（当前通过环境变量配置）
- 小艺用户身份无法映射到 Cat Cafe 用户
...
| OQ-5 | 小艺用户身份如何映射到 Cat Cafe 用户？ | ✅ 已确认：A2A 协议不传用户身份，使用 `DEFAULT_OWNER_USER_ID` 统一绑定（2026-03-27） |
| OQ-6 | `conversationId` 是否跨 app 重启持久？ | ✅ 实测确认：`params.sessionId`（= conversationId）持久不变，顶层 `msg.sessionId` 随 app 重启更换（2026-03-27） |
...
| KD-7 | 用 `params.sessionId` 而非 `msg.sessionId` 作绑定键 | 实测发现顶层 sessionId 随 app 重启变化，params.sessionId 持久 | 2026-03-27 |
| KD-8 | 双 WS 内置于 XiaoyiAdapter，不独立抽 DualWebSocketManager | 代码量可控，减少间接层 | 2026-03-27 |
| KD-9 | `DEFAULT_OWNER_USER_ID` 控制跨渠道 thread 归属 | 与飞书/钉钉/Telegram 共用机制，Web UI 可见性取决于此 | 2026-03-27 |
...
| 2026-03-27 | Phase A 实现：XiaoyiAdapter + bootstrap 集成 + ConnectorDefinition |
| 2026-03-27 | Phase A 实测：发现并修复 7 个 bug（见 Phase A 实测修复记录） |
