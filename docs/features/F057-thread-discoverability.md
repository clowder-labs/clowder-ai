...
# F057: Thread 可发现性 — 排序 + 搜索 + 智能体工具

> **Status**: done | **Owner**: Claude
> **Priority**: P1
> **Completed**: 2026-03-05
> **Evolved from**: F052（跨线程消息暴露了"找不到 thread"的痛点）
> **Related**: F056（设计语言智能体化 — UI 应用新设计标准）

## 愿景

> **一句话**：Thread 多了不可怕，找得到、智能体也找得到才行。

### team experience（2026-03-04）

> "现在 thread 太多了！希望活跃的 thread 自己跑到前面"
> "比如置顶的 有智能体回复我 他也要能跳到上面去"
> "通过 thread id 搜索？不然我找不到！"
> "你们也要有 list_threads MCP 工具 不然如何回答我哪些 thread 举办过智能体杀？"
...
4. 问智能体"哪个 thread 做过 X"，智能体能用 `list_threads` 工具查到并回答
...
| 没有搜索功能 | 只能肉眼滚动找 |
| 智能体没有 list_threads 工具 | 无法回答"哪个 thread 做过 X" |
| 跨线程 badge 只显示 hash 前 8 位 | 认不出来源 thread |
| 智能体不会 @ team lead | team lead不知道哪个 thread 有新动态 |
...
### Phase C
- [x] AC-C1: `list_threads` MCP 工具可用，智能体能按名称搜索 thread
- [x] AC-C2: 智能体能 @ team lead，team lead在 thread 列表看到未读高亮
...
| R2 | "有智能体回复他也要能跳到上面去" | AC-A1 | 发消息后观察排序变化 | [x] |
| R3 | "未读要在前面" | AC-A3 | 截图对比 | [x] |
| R4 | "通过 thread id 搜索" | AC-B1 | 搜索框输入 ID 验证 | [x] |
| R5 | "你们也要有 list_threads MCP 工具" | AC-C1 | 智能体调用工具回答问题 | [x] |
| R6 | "应该增加 at team lead" | AC-C2 | 智能体 @ user + 高亮验证 | [x] |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 排序用最后消息时间，不用 @ 优先 | team lead说"智能体不会 at 我"，目前 @ user 不存在 | 2026-03-04 |
...
- **Related**: F056（设计语言 — UI 遵循智能体化设计标准）
...
- 跨家族 review（Codex codex 或 gpt52）
- Phase B2 的 badge 设计需Gemini视觉确认

## 签收表

| 智能体 | 读了哪些文档 | 三问结论 | 签收 |
|------|-------------|---------|------|
| Claude (opus) | F057 spec, thread-utils.ts, ThreadSidebar.tsx, ChatMessage.tsx, callback-tools.ts, user-mention.ts | 7/7 AC 代码已实现，team lead体验路径完整 | ✅ |
| Codex (codex) | F057 spec, thread-utils.ts:42, ThreadSidebar.tsx:220, ChatMessage.tsx:385, callbacks.ts:230+551, ThreadCatStatus.tsx:45, chatStore.ts:592 | 7/7 AC 独立核实，team lead体验完整 | ✅ |
