...
> **Status**: done | **Completed**: 2026-03-14 | **Owner**: Claude + Codex | **Priority**: P1

## Why

team experience（2026-03-14 00:15~00:25）：

> "以后社区管理能交给你们，PR 和 issue 来了你们知道如何分类，你们知道如何归档，怎么打规范的 tag。知道如何把智能体咖啡的同步到那边，如何把开源的吸收回家。最重要的还有你们要知道我们说 clowder 是开源说智能体咖啡是自己家，别搞混乱。"

> "不是 community 管理，而是一个 skills 里有各个场景的 if 和 ref 到具体的 md，和其他那些 feat 生命周期管理等管理类的 skills 一样。"

F059 建好了技术基础设施（sync 脚本、intake 脚本、ledger 门禁），但智能体缺一个统一的操作规范——现在知识散落在 F059 spec、SOP 段落、讨论稿、脚本注释里，每次操作要自己拼。需要一个 `opensource-ops` skill 把所有场景收敛到一处，加载即可用。

## What

### Phase A: Skill 框架 + 核心场景

建立 `office-claw-skills/opensource-ops/SKILL.md`，覆盖以下场景分支：
...
│   流程：worktree(sync tag) → 修 bug → sync-hotfix.sh → office-claw PR
│         → cherry-pick 回 main → intake record + advance-ledger
│   ref: scripts/sync-hotfix.sh, SOP Hotfix Lane 段落
│
└── 贯穿规则：双仓边界
    office-claw = 家 | office-claw = 开源仓
    哪些操作在哪个仓做，标签在哪里打，PR 在哪里 review
```

**关键设计原则**：

1. **双仓不混淆**：每个操作步骤明确标注在哪个仓执行。feature 标签(`feature:Fxxx`)在开源仓打；BACKLOG 索引在 office-claw 更新。
...
5. **签名归属**：所有开源仓的评论/操作带智能体签名（如 `Codex-gpt5.4`），可追溯。
...
- [x] AC-A1: `office-claw-skills/opensource-ops/SKILL.md` 存在，包含场景 A~F 的完整操作步骤
...
- [x] AC-A7: Skill 加载后，智能体能按场景路由找到对应操作步骤，不需要去翻 F059 spec 或 SOP
- [x] AC-A8: 场景 F（Hotfix Lane）包含：worktree 基于 sync tag 创建、sync-hotfix.sh 用法、office-claw PR 流程、cherry-pick 回 main、intake 登记全链路
- [x] AC-A9: `sync-manifest.yaml` excluded 列表包含 `office-claw-skills/opensource-ops/`，确保 skill 不同步到开源仓
...
| 风险 | 缓解 |
|------|------|
| Skill 写得太长，智能体加载后找不到需要的场景 | 场景路由表置顶，每个场景独立 section，按触发条件快速定位 |
| 双仓操作步骤写错仓库 | 每个步骤强制标注 `[office-claw]` 或 `[office-claw]`，review 时逐条检查 |
...
| KD-2 | Merge 和 Intake 是两个独立 Gate | merge = 接受进开源仓，intake = 决定是否回流家里（Codex-gpt5.4 提议，Claude同意） | 2026-03-14 |
| KD-3 | 所有开源仓操作必须带智能体签名 | 可追溯，防止混淆是谁干的（team lead要求） | 2026-03-14 |
| KD-4 | Patch 类社区 PR 智能体可自主 merge，需同时满足 4 条件 | 条件：① 有 accepted issue ② 只改 safe-cherry-pick 或 public-only 路径 ③ 公开仓 CI/测试过 ④ 不涉及 sync 脚本/ledger/边界规则/安全。碰到 manual-port/Feature/工具链 → 升级team lead（Codex-gpt5.4 提议，Claude同意） | 2026-03-14 |
...
| KD-7 | Skill 写完Codex放行后，@ team lead亲自审核，不直接提 PR | team lead要求 2026-03-14 00:41 | 2026-03-14 |

## Review Gate

- Phase A: Codex review skill 初稿（操作步骤完整性 + 双仓边界正确性）→ 放行后 **@ team lead亲自审核**（不直接提 PR）
- Phase B: Claude review 迁移完整性（diff 对照 community-pr 原文）
