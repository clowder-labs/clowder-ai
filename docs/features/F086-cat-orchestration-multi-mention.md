...
1. 新增 MCP tool handler `office_claw_multi_mention`
...
#### ⚠️ Hindsight (office_claw_reflect) 已废弃
...
### M1 编排运行时
- [x] MCP 工具 `office_claw_multi_mention` 可被猫猫调用
- [x] `targets <= 3` 硬限制
- [x] `parallel only`（首版）
- [x] `callbackTo` 必填
...
9. **F079 Gap 4 与 F086 M1 不混做**：office_claw_start_vote 是投票扩展，multi_mention 是编排运行时，先跑通 M1 再决定 Gap 4 接入方式（gpt52 R3 建议）
