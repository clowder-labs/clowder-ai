...
# F086 Cat Orchestration — 智能体自主协作 + 元认知系统
...
1. **协作链路脆弱**：A2A 经常断线，智能体间协作不流畅，断了就需要team lead手动重新调度
2. **智能体缺少元思考**：不是能力问题，是意识问题 — 智能体不会主动想"这个我应该拉其他猫讨论"
3. **缺少元反思**：做完一个 feat 没有系统性地问"学到了什么？"，反思结果也没有沉淀到能影响未来行为的地方
4. **知识孤岛**：只有Claude有 MEMORY.md，其他猫没有跨 session 记忆

team experience：
> "其实你们经常 a2a 断线，很多时候都是我在强调和要求你们是不是这里能够找其他智能体讨论。"
...
| 缺口 | 问题 |
|------|------|
| 智能体**主动协作** | 不知道什么时候该拉人，工具也不够流畅 |
| **元反思循环** | 做完事不会系统性反思"学到了什么" |
| **知识沉淀** | 反思结果没有变成可复用的知识影响未来行为 |
| **跨猫可检索沉淀** | 只有Claude有 MEMORY.md，其他猫缺少可检索的共享真相源 |
...
当前智能体实际常用的是搜索类工具（search_evidence, session_search, read_session_events）。

**教训**：元反思不能走"自动生成大段反思摘要"的路（token 黑洞）。

#### 活生生的反面教材（Claude自省 2026-03-08）

team lead指出：Claude在采访时没有先搜索了解知识体系现状就开始提问——这恰好是 F086 要解决的"元思考缺失"的活例子。
...
### M1 编排运行时
- [x] MCP 工具 `cat_cafe_multi_mention` 可被智能体调用
- [x] `targets <= 3` 硬限制
- [x] `parallel only`（首版）
- [x] `callbackTo` 必填
...
- **Evolved from**: F079（投票系统 — 智能体协作先例）
...
> 发现时机：F086 feature close 后，team lead在另一线程指出"智能体们对 shared-rules 的注入方式很疑惑——只有一个 link，他们根本不知道还有这玩意"
> 参与者：team lead + gpt52（方案设计）+ opus 4.6（实施简化）
> 决策方式：team lead拍板 quick fix

**问题**：`governance-pack.ts:27` 和 `CLAUDE.md` 都只写了文件路径引用 `office-claw-skills/refs/shared-rules.md`。智能体启动时看不到实际内容，除非主动 `Read` 该文件。F086 M2 给 shared-rules 加了第一性原理和触发器，但注入机制没变——内容更丰富了，猫还是看不到。
