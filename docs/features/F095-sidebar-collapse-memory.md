...
> **Status**: done | **Owner**: Claude | **Priority**: P1
...
team lead反馈：Thread Sidebar 在当前 5-10 个项目时已经很难用（office-claw 174 条 thread 却排在第四位），新建对话窗口太小、不能命名、不能关联 feat、不能置顶。
...
> "默认折叠！分组置顶、拖拽排序等你和Codex一起讨论揣摩一下用户需求都给我优化一下！"
...
├─────────────────────────┤
│ 📁 活跃项目区            │  ← 新增概念
│  office-claw          [▼]  │    近 7 天有活动 OR 用户手动 pin
...
- [x] AC-A4: localStorage key 有命名空间前缀（如 `office-claw:sidebar:`），不与其他功能冲突
...
| R3 | 搜到了但分组折叠看不到（Codex发现） | AC-A5 | test | [x] |
...
- Phase A: Claude实现 → Codex review
- Phase B: Design Gate（前端 UX → team lead确认 wireframe）→ Claude实现 → Codex review
- Phase C: Design Gate（前端 UX → team lead确认 wireframe）→ Claude实现 → Codex review
