...
- 原始扫描结果只保留在当前扫描会话内；只有显式绑定的设备与工作流映射持久保存，TTL 为 0。
...
设备绑定使用持久存储接口；生产实现不得使用仅内存存储。扫描会话、RSSI 样本和未绑定设备信息不进入长期存储。所有写操作在 Core、adapter 与 helper 三层均默认拒绝。

### Phase B: 类型化事件与 Adapter 工具

新增 `LimbEventEnvelope` 与有界事件队列，包含稳定事件 ID、节点 ID、绑定设备 ID、adapter ID、事件类型、观测时间、幂等键和最小 provenance。通知流需要限速、去重和背压，设备断连或通知洪泛不能拖垮 API 进程。
...
- [ ] AC-A2: Core 与 helper 使用版本化、可校验的 NDJSON 协议；helper 异常退出、超时和无效消息不会导致 API 进程退出。
- [ ] AC-A3: macOS CoreBluetooth helper 可完成扫描、显式绑定、连接、标准特征值读取和通知订阅。
- [ ] AC-A4: 未绑定设备不能被猫猫调用；扫描结果、RSSI 样本和未绑定设备信息在扫描会话结束后不保留。
...
- [ ] AC-B1: `ILimbEventSource` 是可选扩展；既有 `ILimbNode` 实现无需修改即可继续工作。
- [ ] AC-B2: `LimbEventEnvelope` 包含稳定事件 ID、节点 ID、绑定设备 ID、adapter ID、事件类型、观测时间、幂等键和最小 provenance。
- [ ] AC-B3: 通知流具有限速、去重、背压与断连恢复；通知洪泛测试不会造成 API 内存无界增长。
...
| KD-6 | 事件能力使用可选 `ILimbEventSource`，不扩大所有 `ILimbNode` 的必选接口 | 保持 F126 既有实现兼容，同时提供类型化事件入口 | 2026-07-14 |
