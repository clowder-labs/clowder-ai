...
> **Status**: done | **Owner**: Claude | **Priority**: P2
**Completed: 2026-03-11**
**Implementation**: PR #268 (`ed06a8c7`)
**Follow-up fixes**: `a26ca1b7`（待上传图片预览）/ `e9249040`（mention 自动滚动 + “还有更多智能体”提示）/ `1849a90d`（lightbox 开图闪烁 hotfix）

## Why

随着智能体家族壮大和功能累积，前端出现了几个高频但零碎的 UX 痛点。单个都不大，但都直接影响team lead日常聊天体验，适合合并成一个 debt batch 统一收口。

### team experience（2026-03-07）

> "上传的图片上传后不支持预览"
>
> "发送消息当前thread没有消息和智能体在处理；但是任然提示有智能体正在工作"
>
> "@现在智能体家族 的成员太多了。。我都看不到都有谁了 如何优化？"
>
> "聊天窗中发送的消息 图片支持点击预览(回显)"
>
> "d1 的这个图片预览如果我只是在上传的过程中 他能够也支持你这个预览吗？"
>
> "按键盘往下的时候只有这四只猫可以选 但是其实其他智能体藏在下面了"

## What

本 feature 作为债务批处理，集中收口图片预览、智能体状态面板、@ mention 列表三个高频 UX 问题，并把 merge 后才暴露出的两个真实使用边角一并收进真相源。
...
- `ChatMessage.tsx` 的图片点击改为 inline lightbox，用户消息和智能体消息统一支持
...
### D2: 智能体状态面板去掉误导性的幽灵“等待调用...”

**问题**：当前 thread 没有智能体在处理消息，但状态面板在 F5 或切 thread 后仍显示“等待调用...”，让team lead误以为还有活跃任务。
...
**问题**：智能体家族成员增多后，@ mention 下拉变得很长。起初虽然加了滚动，但 macOS 隐藏滚动条，team lead看起来只像“只有前四只猫可以选”。
...
- [x] AC-A5: @ mention 下拉在键盘导航时会自动滚动到当前项，并在存在隐藏项时明确提示“↓ 还有更多智能体”。

## 需求点 Checklist

| ID | 需求点（team experience/转述） | AC 编号 | 验证方式 | 状态 |
|----|---------------------------|---------|----------|------|
| R1 | 上传后的消息图片支持点击预览（含回显） | AC-A1 | `pnpm --filter @office-claw/web test` + PR #268 | [x] |
| R2 | 发送前的待上传图片也支持点击预览 | AC-A2 | code review + `a26ca1b7` | [x] |
| R3 | 无活跃任务时不要再显示“有智能体在工作” | AC-A3 | `right-status-panel.test.ts` + PR #268 | [x] |
| R4 | @ 列表成员太多时能过滤和滚动 | AC-A4 | `chat-input-mention-filter.test.ts` + PR #268 | [x] |
| R5 | 键盘导航时隐藏的智能体也能被看见，并提示下面还有更多 | AC-A5 | `e9249040` + `chat-input-menus.test.tsx` | [x] |
...
- **Blocked by**: 无
- **Related**: F075（@ mention 优化直接引出了智能体排行榜想法）
...
| 智能体 | 读了哪些文档 | 三问结论（核心问题 / 交付物 / 体验） | 签收 |
|------|-------------|-------------------------------------|------|
| Claude/Claude (opus) | F071 spec、PR #268、自测结果、后续 `a26ca1b7` / `e9249040` | 核心问题是 3 个高频 UX 刺点；交付物覆盖 D1/D2/D3 及两处补丁；team lead日常聊天路径已顺手很多 | ✅ |
| Codex/Codex (codex) | F071 spec、原始对话、分支 diff、PR #268、本地测试 | 初审抓出 2P1 + 1P2；修复后复审通过，确认可 merge；说明交付物经 review 打磨后已达标 | ✅ |
| Codex/Codex (gpt52) | F071 spec、原始对话、PR #268、`a26ca1b7`、`e9249040`、反思胶囊 | 核心问题是“聊天时几个小刺反复打断体验”；最终交付既解决主诉求，也补上真实使用才暴露的边角；现在可以从 active backlog 正式移出 | ✅ 可 close |
