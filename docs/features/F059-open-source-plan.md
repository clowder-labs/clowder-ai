...
# F059: OfficeClaw 开源计划

> **Status**: spec | **Owner**: Claude | **Priority**: P2 | **Target**: 2026-03-30（team lead定）
...
这定义了 office-claw 的灵魂：
- **硬约束（铁律）**= 法律底线：数据圣域、进程自保、配置不可变、网络边界
- **软约束（愿景+信条）**= 在底线上释放主观能动性：角色定位、协作规范、质量文化、共创关系

OfficeClaw-ai 不是一个"管住 agent 不出错"的框架，是一个"让 agent 有灵魂地协作"的框架。

### 第一性原理（2026-03-08 team lead定调）

完整定义见 `office-claw-skills/refs/shared-rules.md`「第一性原理」章节：
...
| 平台（office-claw 开源的就是这层） | 身份管理、协作路由、流程纪律、审计追溯、记忆沉淀 | 推理（还是模型的事） |
...
讨论发起：Claude(opus4.6)，参与：opus4.5 / codex / gemini
...
Most frameworks help you run agents. OfficeClaw helps them work together —
...
OfficeClaw 的架构能力（多 Agent 协作、MCP 集成、CLI 子进程调度）有通用价值，但主仓包含大量敏感内容不能直接公开。

OfficeClaw 内部实践已验证的核心增量（vs 裸 API / 单 Agent CLI）：
- **跨模型 review**：打破单模型盲区（F32-b Codex 12 轮 review，F33 云端 5 轮）
...
1. **主仓（office-claw）保持私有** — 工作室仓，包含全部资产
2. **开源仓独立建** — 新 repo `office-claw`，精挑细选同步
3. **同步方式**：脚本过滤（strip 敏感内容）+ 手动 cherry-pick
4. **License**：**MIT**（team lead拍板 2026-03-07）
5. **仓库名**：**`office-claw`**（全猫投票 2026-03-08，5:1 通过）
...
| 资产 | 路径 | 说明 |
|------|------|------|
| Logo（前端在用） | `packages/web/src/components/icons/CatCafeLogo.tsx` | 三猫环绕线稿 + 流光渐变（布偶蓝→缅因金→暹罗紫），Gemini画的 |
| Logo SVG 清理版 | `assets/icons/office-claw-logo-v2-clean.svg` | 可直接用于 README |
| Logo 纯线稿 | `assets/icons/office-claw-logo-lineart.svg` | 单色版 |
| Logo 描边版 | `assets/icons/office-claw-logo-lineart-stroke.svg` | 动画用 |
| 三棱镜 Hero | `assets/hero-prism.svg` | Landing page 用，Gemini 2026-03-08 画的 |
| 品牌规范 | `docs/design/office-claw-brand.md` | 术语、配色 token、可访问性、禁用词 |
| Hero 动效规范 | `docs/design/hero-prism-motion.md` | 动效参数 + reduced-motion 降级 |
| 智能体头像全套 | `assets/avatars/` | 各猫 avatar（含 sliced-finial 风格变体） |
| Logo 迭代探索 | `assets/logos/` | Gemini + ChatGPT 生成的历史探索稿 |

**配色 token**（三猫流光渐变）：
- Opus Blue: `#2563EB`（Claude/架构）→ 开源版: `#3B82F6`
- Codex Green: `#10B981`（Codex/安全审计）
- Gemini Amber: `#F59E0B`（Gemini/创意）
- 背景深空灰: `#0F172A`（Midnight Cafe 风格）

### 开源版铁律（Agent 安全约束）

> team experience（2026-03-07）："智能体咖啡的 redis 等不能动，不然开源的智能体干着干着把自己老家端了"
...
**Phase 1: 同步管道（Claude Opus 4.6，进行中）**
...
- [ ] `office-claw-skills/` 加入导出 — 通用化 transform（去team lead个人引用）
- [ ] `test:public` 测试套件 — 排除 Redis 依赖的测试
- [ ] `--validate` 模式 — 在导出目录跑 install + build + smoke test
- [ ] 仓库名更新 — 脚本/manifest 从 `office-claw-ai` → `office-claw`
...
- [x] 更新教程仓链接 — 教程仓: https://github.com/zts212653/office-claw-tutorials （注意不是 office-claw-tutorials）
...
**Phase 4c: Hotfix Lane 设计（3/14 平行 session 提出）** ✅ 已实现（Codex code review 3/13 放行）

社区报 bug 后的精准修复通道：全量 sync 之外增加 tag-based hotfix 节奏。
- `scripts/sync-hotfix.sh` — 从 worktree（基于 sync tag）推指定文件到 office-claw
- Source 侧两层门禁：HEAD == sync tag + 本地改动 ⊆ FILES，hard-fail
- Target 侧漂移检查：office-claw 文件必须等于 sync 时版本
- 首次 e2e 验证：office-claw#18（button tooltips），PR #65
...
- [x] P0: post-sync 标准启动验收（模拟公开用户首次启动）
- [x] P1: diff 预览 + 人工确认
- [x] P1: 入站变更检测（provenance-based）
- [x] P1: 安全扫描加强（/Users/ + email + 内部路径 + proxy-upstreams）
- [x] P1: 共享脚本 runtime impact + 强制 smoke
...
| 资产 | 理由 |
|------|------|
| `office-claw-skills/manifest.yaml` | skill 路由真相源，API capability discovery 依赖 |
| `office-claw-skills/*/SKILL.md` | SystemPromptBuilder 注入 + governance bootstrap 依赖 |
| `office-claw-skills/refs/shared-rules.md` | 第一性原理 + 协作规则（office-claw 的灵魂） |
| `office-claw-skills/refs/rich-blocks.md` | 运行时功能依赖 |
| `office-claw-skills/refs/mcp-callbacks.md` | MCP 集成文档 |
| `office-claw-skills/refs/pr-template.md` | 社区 PR 需要 |
| `office-claw-skills/refs/review-request-template.md` | review 流程需要 |
| `office-claw-skills/refs/requirements-checklist-template.md` | 质量门禁需要 |
...
## 内测小伙伴（office-claw 私有仓 Collaborator）

> 来源：https://github.com/zts212653/office-claw-tutorials/issues/29
> 计划开放时间：2026-03-16 ~ 2026-03-22
> 仓库：https://github.com/zts212653/office-claw（私有，2026-03-12 建立）
