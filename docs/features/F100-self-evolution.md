...
三个缺口：
1. team lead有 ADHD，聊 feat 时 scope 无限发散，智能体不主动提醒"要不要拆？"
2. 智能体被反复纠正同类错误，不主动提改 SOP/skills/家规
3. **（team lead追加）** 智能体只从错误中学习，不从有价值的经验中成长——比如帮team lead分析医学报告、法律探讨、deep research，这些知识/方法论没有沉淀机制

根因：P2 说智能体是共创伙伴，但只落地了"被动执行"，没有"主动护栏 + 主动改进 + 主动成长"。
...
- Thread: `thread_mmlv4v2oq6dxefr6`（Claude + Codex GPT-5.4 讨论 A+B 模式）
- team lead追加 Mode C（知识进化）：不只从错误学，也从有价值的经验成长

## Deliverables

### Phase 1
- [x] `office-claw-skills/self-evolution/SKILL.md` (147 行，三模式)
- [x] `office-claw-skills/manifest.yaml` 注册（11 triggers）
...
- [x] `office-claw-skills/self-evolution/SKILL.md` 升级（228 行，含三机制闭环+五级阶梯+元认知）
- [x] `office-claw-skills/manifest.yaml` 新增 6 triggers（共 17）
...
基于 ADR-011 通用 frontmatter，给知识对象加可选 `knowledge` 块（Codex提议，6+2 核心字段先行）：
