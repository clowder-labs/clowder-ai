...
# F127: 智能体管理重构 — 账户配置与智能体实例分离，支持动态创建猫 + 自定义别名 @ 路由

> **Status**: in-progress (intake + fix done, AC 部分验收) | **Owner**: OpenCode + Codex | **Priority**: P1
...
当前智能体管理是静态 `cat-config.json` 一锅端模式：provider/model/认证/别名/@路由/角色全部写死在一个 JSON 里。用户想加一只新猫或给猫改个 @ 别名，都得手动改配置文件重启。

痛点清单：
1. **账户配置和智能体定义耦合**：provider/model 写在猫的定义里，但认证配置在另一个系统（provider-profiles），两者没有关联
...
5. **Hub 只读**：智能体总览只能看 Provider/Model/Token 限制，不能编辑
...
智能体实例:
  ├── 布偶1 → accountRef: "claude-max",    model: "claude-opus-4-6",   aliases: ["@opus", "@Claude"]
  ├── 布偶2 → accountRef: "my-anthropic",  model: (由 API 决定),        aliases: ["@布偶2", "@Claude二号"]
  ├── 缅因1 → accountRef: "openai-pro",    model: "gpt-5.3-codex",     aliases: ["@codex", "@Codex"]
...
- Hub 账户管理 Tab 重构（从Claude专用 → 通用账户管理）
- 账户 CRUD API

### Phase B: 智能体实例管理（Cat Instance CRUD）

- CatRegistry 支持运行时增删改（保留 cat-config.json 作为 seed/fallback）
- 智能体实例 CRUD API（POST/PATCH/DELETE `/api/cats`）
- 智能体实例与账户配置的引用关系（accountRef）
- 动态创建的猫可以正常被 @ 调用、正常响应
...
- a2a-mentions 同步改动
- @ 自动补全基于智能体实例的 aliases 字段
- API key 接入的猫默认别名包含实际模型名

### Phase D: Hub 智能体管理 UI
...
- [x] AC-A3: 现有 F062 Claude profile 无缝迁移到新账户系统

### Phase B（智能体实例管理）
- [x] AC-B1: Hub 里可以创建新的智能体实例，绑定到某个账户配置
- [x] AC-B2: CatRegistry 支持运行时增删改智能体实例
- [ ] AC-B3: 动态创建的猫可以正常被 @ 调用、正常响应 — **未验证：动态创建的猫需要重启 CLI 才能被 @ 调用**
- [x] AC-B4: 现有预设猫（opus/codex/gemini 等）作为 seed 数据保留，不受影响

### Phase C（动态别名 @ 路由）
- [x] AC-C1: 智能体实例支持自定义别名，@ 路由基于别名工作
- [ ] AC-C2: API key 接入的猫，默认别名包含实际模型名 — **未验证：无 API key 猫的端到端测试**
- [x] AC-C3: @ 自动补全候选列表基于动态 registry

### Phase D（Hub 管理 UI）
- [x] AC-D1: Hub 智能体总览支持新建/编辑/删除智能体实例
- [x] AC-D2: 智能体编辑支持修改别名、昵称、角色描述、切换账户

## Dependencies

- **Evolved from**: F062（Claude账号配置中枢 — Anthropic-only provider profile）
- **Related**: F032（Agent Plugin Architecture — CatId 松绑 + CatRegistry 基础）
- **Related**: F050（External Agent Onboarding — A2A/CLI 接入契约）
- **Related**: F105（OpenCode接入 — opencode provider 注册模式参考）
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 两层分离：账户配置 × 智能体实例 | 认证凭据和智能体身份是正交关注点，解耦后可独立扩展 | 2026-03-17 |
| KD-2 | 持久化用文件（`.office-claw/cat-catalog.json`），不用 Redis | 社区 PR #130 选择了文件方案。原因：开源用户不一定有 Redis；文件方案零外部依赖。**代价**：引入了"两个 JSON 文件存同类数据"的双真相源风险。已通过 PR #632 的 deep merge 缓解（catalog 只是 delta overlay，不再整体替换 config） | 2026-03-21 |
...
| R-4 | **智能体模板机制未做** — 社区 issue 里提到的"预设品种→一键创建变体"能力，当前 Hub 只有完全手动填表 | 低（非 MVP 范围） | 用户量增长后 onboarding 体验优化 | 未来 Feature |
...
| R-10 | **本地反代 `anthropic-proxy.mjs` 的 upstream 配置未初始化** — `start-dev.sh` 启动的反代（端口 9877）依赖 `.office-claw/proxy-upstreams.json` 配置上游，但 F127 intake 后 runtime 里该文件不存在。API key profile 创建应自动注册 upstream 到反代 | 中（反代功能不可用） | 配置 API key profile 用本地反代时 | profile 创建/更新时自动写 `proxy-upstreams.json` |
