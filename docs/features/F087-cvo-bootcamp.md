...
# F087: 智能体训练营（CVO Bootcamp）

> **Status**: done | **Owner**: Claude | **Completed**: 2026-03-12
> **Priority**: P2
> **Evolved from**: F059（开源计划 — office-claw 需要 onboarding 体验）
> **Related**: F075（成就/排行榜系统）, F096（可交互富文本）

## Why

office-claw 开源后，用户拿到框架但不知道怎么用。文档教程是"读"，训练营是"做"——用户在智能体陪伴下走完一次真实的 feat lifecycle，从此知道如何当一个合格的 CVO（Chief Vision Officer）。

### team experience（2026-03-08）

> "智能体训练营！如何快速培养一个合格的 CVO team lead？可以借用游戏的新手任务模式呀！你们几只可爱大智能体带大家使用我们的智能体咖啡！比如一起研发一个有趣的功能，走我们完整的 feat 流程体验一下协作的乐趣！当然不能是贪吃蛇这类简单的要死的。得有 UX UI 让他们看看你们的实力？！"

### team lead纠正（2026-03-10）

> MVP **不是**我们内置任务让用户做，而是引导新用户**像team lead一样和智能体协作**——帮装 MCP、解决配置问题、带走一次真正的 feat lifecycle。成就系统直接接入 F075 智能体排行榜，不要重新发明。**家规 P1：每步产物是终态基座不是脚手架。**

## What

### 核心概念

**Onboarding Flow + 游戏化引导**：用户不是在读教程，是在智能体陪伴下**配好环境、学会协作、真正造一个功能**。

| 要素 | 游戏类比 | 训练营实现 |
|------|---------|-----------|
| 新手村 | Starting zone | 环境检测 + MCP 安装 + 配置帮助 |
| 引导 NPC | Tutorial NPC | 智能体天团**轮流登场自我介绍** + 分角色引导 |
| 主线任务 | Story quest | 和智能体一起完成一个完整 Feature |
| 成就徽章 | Achievements | 接入 **F075 智能体排行榜**（"第一次拍板"、"第一次否决"等）|
...
用户点击"引导"按钮 / 说"我是新手"
  ...
  → Phase 1: 智能体天团轮流登场自我介绍（Claude→Codex→Gemini）
  → Phase 2: 环境检测——MCP 装了吗？配置对吗？缺什么依赖？
  → Phase 3: 智能体主动帮用户解决配置问题（不是甩文档链接！）
  → Phase 3.5: 进阶功能引导（TTS/ASR/Pencil，跑不起来就跳过）
  → Phase 4: 展示任务候选菜单，用户选一个感兴趣的
  → Phase 5: 立项——用户说愿景，智能体帮结构化成 spec
  → Phase 6: 设计讨论——智能体各自出方案，用户做 CVO 拍板
  → Phase 7: 开发——智能体写代码，用户做方向判断
  → Phase 8: Review——跨猫 review，用户看到意见分歧+收敛过程
  → Phase 9: 完成——功能上线，用户的名字在 commit 里
  → Phase 10: 回顾——智能体带用户复盘，成就写入 F075 排行榜
  → Phase 11: 告别——告诉用户以后有问题可以回这个训练营线程找智能体
```

### 前端入口（Design Gate 2026-03-11 确认）

- **Sidebar 按钮**：智能体学士帽 SVG icon，放在 "+ 新对话" 按钮旁边（不用 emoji，自己画）
- **空消息态 CTA**：在现有欢迎文字下方加 "第一次来？开始智能体训练营" 引导链接
- 点击任一入口 → 自动创建新 thread（title: "🎓 智能体训练营"）→ 进入 Bootcamp 模式
- 配套 Skill：`bootcamp-guide`（智能体进入引导模式，更耐心、更多解释、主动检测环境）
...
|------|------|---------|-------------------|
| **TTS（语音合成）** | 智能体能说话 | 推荐 **Kokoro-82M**（82M 参数，多数机器跑得动）；我们自己用 Qwen3-TTS 1.7B（音质最好但吃资源） | 无语音，纯文字交互 |
...
训练营完成后，这个线程不会消失——**用户以后有问题可以回到训练营线程找智能体**。

- 完成回顾后，智能体告知用户："以后有什么需要帮助的，随时回这个线程找我们！"
- 训练营线程保持 pinned 状态，方便用户找到
- 智能体在这个线程里持续保持"引导模式"（更耐心的回复风格）
...
| # | 任务名 | 提出者 | 亮点 | 涉及能力 |
|---|--------|--------|------|---------|
| Q1 | **智能体盲盒** — 每天随机生成一只"每日智能体"，有独特名字+性格+趣味建议 | Claude(opus) | 即时惊喜感，每次打开都不一样 | 前端动画 + 随机生成算法 + 智能体人设系统 |
| Q2 | **智能体星座运势** — 输入生日，智能体们用各自风格解读今日运势 | Claude(opus) | 个性化 + 多猫不同解读对比 | 多 Agent 独立生成 + 前端卡片 UI |
| Q3 | **智能体侦探社** — 给一段"代码案发现场"，智能体们各自推理凶手（bug），用户做裁判 | Claude(opus) | 游戏化 debug 体验 | 多猫推理 + 用户决策 + review 流程 |
| Q4 | **心情墙/留言板** — 用户发心情，智能体用各自风格回应（Claude讲道理、Codex找 bug、Gemini画猫） | Claude(opus) | 情绪价值拉满，即时互动 | 前端瀑布流 + 多 Agent 回复 + 存储 |
| Q5 | **Emoji 制造机** — 描述一个表情，智能体合作生成自定义 emoji（暹罗画+布偶调+缅因审） | Gemini(gemini) | 创意表达 + 跨猫协作可视化 | 图像生成 + 跨猫 review + 前端展示 |
| Q6 | **智能体拿铁实验室** — 选配料（咖啡因/奶泡/糖浆），智能体各自推荐配方，用户拍板尝试 | Codex(codex) | 轻松有趣，咖啡馆主题契合 | 前端配方 UI + 多猫推荐算法 + 决策流 |
| Q7 | **智能体点餐系统** — 给虚拟OfficeClaw设计菜单+点餐流程 | Claude(opus) | 咖啡馆主题天然契合，所见即所得 | 全栈：菜单 CRUD + 点餐 UI + 结算逻辑 |
| Q8 | **像素智能体世界** — 像素风格的智能体互动场景，team lead可以操作智能体移动/互动 | team lead | 即时视觉反馈，游戏感强 | Canvas/像素渲染 + 键盘交互 + 智能体 AI 行为 |
| Q9 | **3D 智能体能力看板** — 3D 或 2D 动态可视化，看到不同智能体在做什么、各自的能力雷达图 | team lead | 纯视觉 + 立刻反馈，不是严肃 Mission Hub 而是好看的智能体们 | Three.js/CSS 3D + 实时数据 + 动画 |
| Q10 | **智能体互动玩具** — 前端游戏化界面，team lead可以和智能体们做小互动（逗猫棒、喂食、摸头） | team lead | 纯情绪价值，建立和 AI 团队的情感连接 | 前端交互动画 + Agent 性格化回应 |

#### Lv.2 — 有深度但仍有趣

| # | 任务名 | 提出者 | 亮点 | 涉及能力 |
|---|--------|--------|------|---------|
| Q11 | **智能体天气站** — 输入城市，三只猫用不同风格播报天气（Claude分析气压、Codex安全提醒、Gemini画天气图） | Codex(codex) | API 集成 + 多猫个性化输出 | 外部 API + 多 Agent + 前端天气 UI |
| Q12 | **每日 Standup 面板** — 智能体们每天自动生成"今天我干了什么"的可视化面板 | Gemini(gemini) | 展示协作可观测性 | 数据聚合 + 可视化 + 定时任务 |
| Q13 | **智能体成就博物馆** — 展示所有智能体的历史成就、里程碑、有趣的 commit message | Gemini(gemini) | 回顾感 + 荣誉感 | Git 数据挖掘 + 前端展览 UI + 筛选过滤 |
| Q14 | **智能体翻译官** — 用户输入一段话，智能体们各自翻译成不同"风格"（正式/卖萌/技术/诗意） | Codex(codex) | 展示多 Agent 个性差异 | 多 Agent 并行 + 风格对比 UI |

#### Lv.3 — 进阶协作挑战

| # | 任务名 | 提出者 | 亮点 | 涉及能力 |
|---|--------|--------|------|---------|
| Q15 | **决策室（Decision Room）** — 给一个真实的技术/产品二选一，智能体各自论证，用户做最终裁判 | 全猫共识 | 展示跨猫 review + 意见分歧收敛的核心价值 | 多猫独立论证 + 结构化对比 + 用户裁决 + 复盘 |
| Q16 | **智能体代码接力** — 一只猫写骨架，另一只猫加功能，第三只猫 review，用户做方向裁判 | Claude(opus) | 完整体验多猫分工协作 | 全 feat lifecycle + 跨猫 handoff + review |

> **team lead补充（2026-03-08）**：候选不限于以上！任何有趣的互动形式都可以加入。核心原则：
> 1. 新手team lead**自己选**感兴趣的任务（菜单制，不是单一路径）
> 2. 好玩优先，让人想继续玩下去
> 3. 每个任务都能体验到"和智能体一起造东西"的乐趣
> 4. 视觉反馈要即时——做完就能看到成果
...
| 级别 | 能力 | 训练营体现 |
|------|------|-----------|
| Lv.1 | 表达愿景 | 用户口述想要什么，智能体帮结构化成 spec |
| Lv.2 | 判断方案 | 智能体出 2-3 个方案，用户选择并说理由 |
| Lv.3 | 纠偏 | 开发中途智能体故意问"要不要加 X？"，用户判断是否跑偏 |
...
- [x] AC-A2: 智能体天团轮流登场自我介绍（Claude→Codex→Gemini，各自风格）（bootcamp-guide skill Phase 1 引导）
...
- [x] AC-A8: 成就/进度接入 F075 智能体排行榜系统（终态基座，不搞临时版）（PR #391）
- [x] AC-A9: 训练营可作为 office-claw 的 Quick Start 引导（**Deferred to F059** — office-claw 开源后对接，训练营代码已就绪）
...
- [x] AC-A12: 训练营完成后线程保持可用，用户以后可回来找智能体求助（GET /api/bootcamp/thread 线程发现 + auto-pin）
...
| 屏 | 内容 | 设计要点 |
|----|------|---------|
| Screen 1 | 入口（Sidebar + 空消息态） | SVG 智能体学士帽按钮放 "+ 新对话" 旁；空消息态加 CTA |
| Screen 2 | Phase 0-1（选引导猫 + 天团登场） | 用户先选主引导猫（card-grid），然后三猫依次自我介绍 |
| Screen 3 | Phase 2-3（环境检测 + 配置帮助） | checklist 卡片（✅/⚠️/❌），有问题智能体主动给解法 |
| Screen 4 | Phase 4（任务选择菜单） | card-grid 按难度分三层 + "🎲 随机抽" 按钮 |
| Screen 5 | Phase 5+（进入真实协作） | 进度条 Rich Block + 正常智能体协作，只是更耐心 |
...
- Review：Codex 3 轮 review（P1 跨线程写入 + P1 default bypass + P2 stale guard）

### Phase C: 运行时编排 ✅ (PR #386, `725a24bc`)

**交付物**：
- `SystemPromptBuilder.ts`: `threadId` 注入 bootcamp prompt（智能体可调 MCP 工具）
...
- Review：Codex 1 轮放行（0 P1/P2）+ 云端 Codex 放行
...
- Review：Codex 3 轮 review（P1 phase-skip farming + P1 dead def + P2 event contract + P2 response validation）+ 云端 Codex LGTM
...
- 仅 AC-A9（Quick Start 文档）待 office-claw 开源后对接
...
| KD-5 | 成就系统直接接入 F075 智能体排行榜，不重新发明（家规 P1 终态基座） | 2026-03-10 | team lead |
| KD-6 | 智能体天团登场方式：轮流自我介绍（不是同时说话） | 2026-03-10 | team lead |
| KD-7 | 前端入口：在现有聊天界面加"引导"按钮，不做独立页面 | 2026-03-10 | team lead |
| KD-8 | 入口按钮用自绘 SVG icon（智能体学士帽），不用 emoji；放在 "+ 新对话" 旁边 | 2026-03-11 | team lead |
| KD-9 | 让用户选择主引导猫（Claude/Codex/Gemini），考虑不同模型成本差异 | 2026-03-11 | team lead |
| KD-10 | 任务选择菜单加"随机抽"功能（骰子动画） | 2026-03-11 | team lead |
| KD-11 | 渐进式引导：Phase 0-3 强引导（智能体手把手），Phase 4+ 回归真实协作体验 | 2026-03-11 | Claude+team lead |
| KD-12 | 环境检测 MVP 最小集：Node.js + pnpm + Git + Claude CLI + MCP 连接状态 | 2026-03-11 | Claude |
| KD-13 | 任务选择菜单和引导猫选择使用 Interactive Rich Blocks（F096），通用可交互富文本组件 | 2026-03-11 | team lead+Claude |
...
| KD-16 | 训练营线程作为持续帮助入口——完成后不消失，用户可回来找智能体 | 2026-03-11 | team lead |

## Dependencies

- **Evolved from**: F059（开源计划 — office-claw 需要 onboarding 体验）
- **Blocked by**: F096（Interactive Rich Blocks — 任务选择/引导猫选择需要可交互富文本）
- **Related**: F075（智能体排行榜 — 成就/统计系统的真相源）
- office-claw 核心框架就绪后才能做训练营
...
| 引导太死板像教程 | 智能体的回复保持个性和自然感 |
| 环境千差万别导致配置失败 | 智能体主动诊断 + 提供多种解法 |
