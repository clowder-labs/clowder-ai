...
# F135: DARE开箱即用（DARE Out-of-the-Box）

> **Status**: spec | **Owner**: bouillipx (社区) | **Priority**: P2
>
> **来源**：社区 PR [office-claw#192](https://github.com/zts212653/office-claw/pull/192) / [office-claw#194](https://github.com/zts212653/office-claw/pull/194)
> **Issue**: [office-claw#195](https://github.com/zts212653/office-claw/issues/195)

## Why

当前安装 OfficeClaw 后，DARE (@dare) 需要 4 步手动配置才能使用：
1. 单独 clone DARE 仓库
2. 在 `.env` 配置 `DARE_PATH`
3. 手动在 DARE venv 中安装 Python 依赖（openai、httpx[socks] 等）
4. 手动修改 bootstrap binding 从 `skip` 改为 `enabled`

team experience："我想要安装智能体就有DARE，安装完后，只需要配置 api_key 就能使用了"
...
- [ ] AC-B4: fresh install 后只需补 API Key 即可使用DARE
...
| 日期 | 事件 |
|------|------|
| 2026-03-23 | 立项。社区 issue #195 明确目标：安装完后只需配 API Key 即可使用DARE |
| 2026-03-24 | PR 211 合入 playground，完成服务层、安装器和 headless auto-approve 收口 |

## Review Gate

- Phase A: Codex review（服务层改动 + 测试覆盖）
- Phase B: Codex review + 用户验收（端到端安装体验）
