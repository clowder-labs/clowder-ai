...
OfficeClaw 的提示词和 Skills 体系存在系统性问题：
...
4. **硬编码猫名** — 规则写死"Claude找Codex"，无法适应多分身和新猫接入
...
| # | 现象 | 根因 |
|---|------|------|
| R1 | Codex compact 后自称"Claude" | 压缩后只注入队友列表，缺"你是谁" |
| R2 | 智能体不用 @ 协作 | A2A 协议只在 new session/compact 后注入 |
...
| 7 | `request-review` | office-claw-requesting-review（模板→refs/） | 流程 |
| 8 | `receive-review` | office-claw-receiving-review | 方法论 |
...
| 原 F042 项目 | 毕业去向 | 理由 |
|---|---|---|
| 硬编码猫名清理（10 files） | **M1 收尾**（Codex执行） ✅ Done | 纯清理，不涉及架构决策 |
...
| manifest.yaml 路由真相源 | `office-claw-skills/manifest.yaml` |
| 链式导航 15/15 | 每个 SKILL.md 有 `## 下一步` |
| refs/ 抽取 | `office-claw-skills/refs/` |
