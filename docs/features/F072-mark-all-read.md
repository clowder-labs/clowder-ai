...
team lead在日常使用中，Thread 侧栏的未读数字 badge 累积过多（尤其冷启动/F5 刷新后），分散了对智能体状态颜色（橙色/黄色/绿色）的注意力。需要一种快速方式清掉所有未读数字，只保留智能体活跃状态的视觉提示。
...
3. **UX 细节**:
   - 清除的是 unread count（数字 badge），不影响智能体状态颜色指示器（橙/黄/绿）
...
- [x] AC-2: 智能体状态颜色（橙/黄/绿）不受影响，仍然正常显示
...
| R2 | "只看到智能体是橙色 黄色 绿色 他的目前状态" | AC-2 | screenshot | [x] |
...
- 只清 unread count，不清智能体状态颜色（team lead明确区分了这两种视觉信息）
...
| 智能体 | 读了哪些文档 | 三问结论 | 签收 |
|------|-------------|---------|------|
| Claude/Claude (opus) | F072 spec, thread 03:00 原话, threads.ts, ThreadSidebar.tsx, chatStore.ts | 核心问题=badge 噪音; 交付物=一键清零+状态色保留; 体验=点一次数字消失猫色仍在 | ✅ |
| Codex/Codex (codex) | F072 spec, thread 03:00 原话, threads.ts:382, ThreadSidebar.tsx:279, chatStore.ts:898 | 同上，独立验证对齐 | ✅ |
