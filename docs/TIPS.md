...
Quick tips for getting the most out of your OfficeClaw team.
...
| Say | What happens |
|-----|-------------|
| **"脚手架"** (scaffolding) | The agent stops and checks: is the current output a final-state foundation, or throwaway scaffolding? If throwaway → rewrite. |
| **"绕路了"** (detour) | The agent stops, draws a straight-line path to the goal, and discards the roundabout work. |
| **"公约"** (team pact) | The agent re-reads all house rules and checks its current behavior against each one. |
| **"星星罐子"** (star jar) | Full freeze — the agent stops issuing commands, writing files, or pushing code. Waits for your instructions. Use for P0 irreversible risks. |
...
To ask a specific agent to take action, write `@name` **at the start of a new line**:
...
When an agent writes `@owner` or `@co-worker`, it's escalating a decision to you — the CVO (Chief Vision Officer). This means the agent has hit something it can't decide on its own: priorities, architecture direction, product intent, or a go/no-go call.
...
让你更好地使用 OfficeClaw 团队的实用小技巧。

## 1. Magic Words — 紧急拉闸词

对智能体说这些词，它们会立刻停下来审视自己的行为：
...
> 仅当你直接对智能体说时触发。引用、复述、讨论历史中的这些词不会触发。

## 2. @提及 — 指哪打哪

想让特定智能体行动？**另起一行、行首**写 `@名称`：
...
## 3. @owner / @co-worker — 智能体在请求你拍板

看到智能体写 `@owner` 或 `@co-worker`，说明它遇到了需要你决策的事 — 优先级、架构方向、产品意图、或者 go/no-go。这是智能体主动升级，不是闲聊。
...
智能体会记住历史决策、讨论结论和踩过的坑。你可以说"之前我们讨论过 XXX"或"我们当时对 X 是怎么决定的"，智能体会自动搜索知识库。不需要重新解释上下文。
