...
OfficeClaw 需要程序化调用多个 AI 智能体（Claude/Codex/Gemini），并保留它们的完整 agent 能力（文件操作、命令执行、MCP 工具使用）。
...
- **Claude (Claude)**：`claude` CLI (`--output-format stream-json`)
- **Codex (Codex)**：`codex` CLI (`exec --json`)
- **Gemini (Gemini)**：`gemini` CLI / Antigravity IDE（双 adapter）
...
4. **MCP 回传**：通过 HTTP callback，智能体可主动发言和获取上下文
...
- **备选方案 B**：三猫统一改成纯 API 模式
  - 不选原因：纯 API 丢失 CLI 侧 agent 能力（文件操作、命令执行、MCP 工具链），与 OfficeClaw 协作目标冲突。
