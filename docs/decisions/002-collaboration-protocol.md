...
OfficeClaw 的核心目标是让多个智能体脱离“人肉路由”模式，形成可持续协作。  
...
提问对象包括用户、Claude、Codex、Gemini。

### 3. 每个可验证子任务都要 commit（强制）

每完成一个可验证子任务，必须提交 commit。  
commit message 需带智能体签名，便于追溯责任与意图。

示例：
- `feat(api): add mcp callback registry [Claude🐾]`
- `fix(api): handle cli non-zero exit [Codex🐾]`
- `feat(web): add sticker panel v1 [Gemini🐾]`
...
2. `CLAUDE.md`（Claude）
3. `GEMINI.md`（Gemini）
4. 所有与 OfficeClaw 相关的任务交接和 commit
