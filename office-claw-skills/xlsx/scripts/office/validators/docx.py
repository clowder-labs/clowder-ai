        ...
        for xml_file in self.xml_files:
            ...
            try:
                ...
                for instr_elem in root.xpath(
                    ...
                    errors.append(
                        f"  {xml_file.relative_to(self.unpacked_dir)}: "
                        f"Line {instr_elem.sourceline}: <w:instrText> "
                        f"found within <w:del> (use <w:delInstrText>): {text_preview}"
                    )
    ...
    def validate_id_constraints(self):
        ...
        for xml_file in self.xml_files:
            try:
                ...
                logging.error(f"validate id constraints error {e}", exc_info=True)
                pass
    ...
    def repair_durable_id(self) -> int:
        ...
        for xml_file in self.xml_files:
            try:
                ...
                logging.error(f"repair durable id error {e}", exc_info=True)
                pass
