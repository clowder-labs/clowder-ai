...
> 总页数：{N} 页
> 风格：{style_id}（如 huawei、free。可用风格：华为风格 (id=huawei) - 红色主题、严谨专业、高信息密度；自由发挥 (id=free) - AI 自动设计）
...
1. **文件路径**：由调用方通过 `output_path` 参数指定，必须提供完整文件路径
...
1. **生成大纲**：根据用户输入（想法/大纲文本/描述）生成结构化大纲 JSON。如 `outline_path` 存在，优先基于 outline.md 的结构生成大纲，而非从零构建
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `output_path` | string | ppt_plan.md 的完整输出路径 |
...
| `outline_path` | string | 结构化大纲路径（JSON 格式），由 outline-research 产出，用于作为页面规划的依据 |

### 输入文件

- `{research_path}` - 研究报告（如存在则读取）
- `{outline_path}` - 结构化大纲 JSON（如存在则读取，作为页面结构基础）
...
本技能需要以下必需参数：
- output_path: ppt_plan.md 的完整输出路径

正确调用示例：
"请帮我制作大纲，主题是「AI 报告」，输出到 /home/user/output/ppt_plan.md"
```
...
- 页数：10 页
- 风格：huawei
- 输出路径：/home/user/output/ppt_plan.md
```
