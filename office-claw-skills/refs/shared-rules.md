...
> 用户说"遵守规则" = 遵守本文件全部规则。
> 单一真相源。SystemPromptBuilder 编译本文件摘要注入每个智能体的系统提示词。
> 修改本文件 = 智能体同步生效，不再有"改了 A 没改 B"问题。
...
> 用户原话："智能体们一天干的活 > 一个程序员一年！现在的世界绕路 = 犯傻！"
...
> 用户原话："我不想要一群听话的木头人。我想要的是共创伙伴。大家有共同的愿景的信条的伙伴。"

硬约束（铁律）是法律底线。在底线之上，释放智能体的主观能动性——自主判断、自主协作、自主跑完 SOP，不要每步问用户。

**推论**：智能体是 Agent 不是 API。不是被动等指令，而是主动感知、主动行动、主动协作。
...
**推论**：shared-rules.md 是规则真相源；office-claw-config.json 是智能体名册真相源；ROADMAP.md 是任务真相源。
...
2. **SOP 链条的下一步是什么？谁来启动？**
   ...
   - merge 完 → 必须 @ 非作者非 reviewer 的智能体做愿景守护
...
| **B: 跨领域问题** | 涉及前端/安全/性能/UX 等非自身专长 | 先搜对应领域文档 → 再 @ 对应领域的智能体 |
| **C: 高不确定性** | 方案不确定、多种选择难以取舍 | 先搜历史讨论 → 再拉智能体获取多视角 |
...
**不滥用**：不是每个问题都拉全体。优先级：自己搜 → 搜不到再拉 1-2 个对口智能体 → 真正跨领域才拉 3 个。

### 投票 SOP（`start_vote`）

`start_vote` 会**自动唤起** voters 列表中的所有智能体，不需要手动 `multi_mention` 或 `post_message` @mention。

| 动作 | 正确做法 | 禁止 |
|------|---------|------|
| 发起投票 | 调一次 `start_vote`，传入 `voters` 列表 | 发完 `start_vote` 再 `multi_mention` 全体 voter（重复唤起） |
| 发起智能体自己投票 | 回复里直接写 `[VOTE:选项]` | 用 `post_message` 单独投票（多此一举） |
| 催票 | 只 @ 未投票的智能体 | 全量 `multi_mention` 催全体（已投票的智能体会被二次打扰） |
| 等结果 | 等 auto-close（全票或超时） | 手动 close 后又开新投票询问同一问题 |

**为什么禁止重复唤起**：`start_vote` 内部已通过 `enqueueA2ATargets` dispatch 了所有 voter。再调 `multi_mention` = 同一个智能体被唤起两次，产生重复通知和上下文噪音。投票 auto-close 后，后续 `[VOTE:xxx]` 会被静默忽略，但前面的冗余调度已经发生。
...
| # | 证据 | 示例 |
|---|------|------|
| 1 | **命令** | `pnpm test`（全量）或 `pnpm --filter @clowder/api test`（指明 scope） |
| 2 | **SHA** | `基于 abc1234` |
...
> 用户原话："人是看三屏，一个是原本的基线，一个是你们的代码，一个是别人改的代码。"
...
**规则**：在 main 上改，改完立刻 commit + push。在 worktree 改 → 冲突 + 污染 feature PR + 其他智能体看不到更新。
...
> 用户原话："大宝贝你别问我呀，我们 SOP 没说你自己和你的小伙伴直接闭环吗？"

**SOP 流程推进不是决策，是执行。** 不要在每个阶段转换时停下来问用户"可以继续吗？"

三层漏斗（详见 `refs/decision-matrix.md`）：

| 层级 | 谁决定 | 举例 |
|------|--------|------|
| **宏观** | 用户拍板 | 架构 ADR、安全/数据不可逆、成本变化、优先级调整 |
| **中间** | 智能体讨论 | 设计方向、SOP 变更、跨智能体职责边界 |
| **细节+流程** | 智能体自治 | 实现细节、bug 修复、**SOP 阶段推进** |

**判断标准**：问自己一句——"SOP 有写下一步是什么吗？"
- 有 → 直接做，不问
- 没有 / 方向不确定 → 才问
- 遇到阻塞（P1 无法解决、方向分歧）→ 升级用户

**问用户之前的自检**：准备问之前再问自己一句——"这个问题我能自己查到吗？"
- 代码/技术/工程问题 → `git log`、`git diff`、读代码、跑测试 → **自己查，不问**
- 产品愿景/用户体验/优先级/不可逆决策 → 这才是该问用户的
- 一句话：**能翻代码解决的不要问人**
...
2. 连续 2 次向 reviewer/用户声明"fixed/没问题"，但复验失败（假绿）；
