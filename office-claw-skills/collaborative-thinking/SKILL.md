...
description: >
  单人或多 agent 的创意探索、独立思考、讨论收敛。
  Use when: brainstorm、多 agent 独立思考、讨论结束需要收敛、方向性问题需要多视角。
  Not for: 已有明确 spec 直接写代码、单 agent 执行已定方案。
...
三种思考模式：单人探索 / 多 agent 独立思考 / 讨论收敛沉淀。通用思考框架。
...
| **C 收敛沉淀** | 任何讨论产出了决策/规则/否决理由 | 纯问答（结论在 thread 里已够）、用户说"不用记" |
...
Phase 6: 用户反馈 + 最终确认 → 进入 Mode C
```

**Phase 1 独立性保护规则（最重要）**：
- 禁止互看：每个 agent独立完成，不预测他人观点
- 防锚定：有背景材料时，先形成自己想法再参考
- 展示推理链："我为什么这么想"，不只给结论
- 标注不确定性：区分确信的结论和猜测

实现方式：`routeParallel()` 或用户分别 @ 各 agent并强调"先独立思考"。

**Phase 2 触发**：各方基本一致 → 跳过；存在明显分歧 → 需要（限 2-3 轮）；用户说"够了" → 跳过。

**Phase 4 综合必须包含**：各方观点摘要 / 共识区 / **分歧区**（不要抹平！）/ 待决事项 / 行动项。

**扇入者默认**：Brainstorm 类 → 用户；技术讨论 → 指定综合者 + 指定把关者。用户可随时覆盖。
...
| Mode B Phase 1 让 agent 看到彼此回答 | routeParallel 或分别 @ 并强调独立思考 |
| Mode B 综合时抹平分歧 | 分歧必须保留 + 标注各 agent 理由 |
