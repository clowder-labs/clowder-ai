...
**页数 + 风格收集**：当用户已提供主题但缺少页数和/或风格时，一次性收集两项信息。

**方式一：使用 AskUserQuestion 工具**（如可用）
...
**方式二：纯文本交互**（AskUserQuestion 工具不可用时）

使用纯文本一次性询问页数和风格，用户通过编号或自描述回复。
等待用户回复后解析 `{page_count}` 和 `{style_id}`。
