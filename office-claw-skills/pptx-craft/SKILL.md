---
name: pptx-craft
description: "PPT 主流程技能，通过多 Agent 协作完成研究、规划、生成全流程。支持用户上传文档（docx、pdf、md、图片等）自动解析内容后生成 PPT。"
metadata:
  dependencies:
    - "./planner/SKILL.md"
    - "./designer/SKILL.md"
...
| **Eve**（文档解析师） | 文档内容解析专家 | 解析用户上传的文档（docx/pdf/md/图片等），提取原文，输出 doc_raw.md | Agent tool (general-purpose) |
...
- 新建 PPT
- 基于主题 / 材料生成演示文稿
- **上传了文档（docx、pdf、md、图片等）并要求生成 PPT**
- 修改已有大纲 / 页面结构 / 文案方向
- 在已生成产物上继续迭代内容

如果只是普通问答、寒暄、纯事实查询，不进入 PPT pipeline。

**文档上传检测**：如果用户消息中附带了文件（docx、pdf、md、txt、图片等），或引用了文件路径，视为"基于文档生成 PPT"的请求，自动进入 PPT pipeline 并触发文档解析流程。
...
| 文档类型 | 扩展名 | 解析方式 |
|---------|--------|----------|
| Word 文档 | `.docx`, `.doc` | 使用 docx 技能（`{skill_root}/docx/SKILL.md`）解析，完整提取正文、标题层级、表格等内容 |
| PDF 文档 | `.pdf` | 使用 pdf 技能（`{skill_root}/pdf/SKILL.md`）解析，完整提取文字、结构和关键信息 |
| Markdown 文件 | `.md` | 使用 Read 工具直接读取，保留标题层级和结构 |
| 纯文本 | `.txt` | 使用 Read 工具直接读取全文 |
| 图片 | `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp` | 使用图片解析技能（`{skill_root}/image/SKILL.md`）解析，识别并提取图中文字、图表和关键信息 |
...
1. **读取文档**：根据文件扩展名选择解析方式：
   - `.docx` / `.doc` 文件 → 读取 docx 技能（`{skill_root}/docx/SKILL.md`），按照技能指引解析文档
   - `.pdf` 文件 → 读取 pdf 技能（`{skill_root}/pdf/SKILL.md`），按照技能指引解析文档
   - `.md` / `.txt` 文件 → 使用 Read 工具直接读取文件内容
   - `.png` / `.jpg` / `.jpeg` / `.gif` / `.webp` 图片文件 → 读取图片解析技能（`{skill_root}/image/SKILL.md`），按照技能指引识别并提取图中文字、图表和关键信息
...
- docx 文件必须使用 docx 技能解析，pdf 文件必须使用 pdf 技能解析，图片文件必须使用 image 技能解析，不要直接用 Read 工具读取
- txt 和 md 文件使用 Read 工具直接读取即可
