...
function isValidBorder(side) {
  return side.width > 0 && side.color !== null && side.style !== "none" && side.style !== "hidden";
}
...
export function generateCompositeBorderSVG(w, h, radius, sides) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><defs><clipPath id="${clipId}"><rect x="0" y="0" width="${w}" height="${h}" rx="${adjustedRadius}" ry="${adjustedRadius}" /></clipPath></defs><g clip-path="url(#${clipId})">${borderRects}</g></svg>`;
...
export function getTextStyle(style, scale) {
  ...
  return {
    ...
    ...(() => {
      ...
      return bgColor.hex && bgColor.opacity >= 0.3 ? { highlight: bgColor.hex } : {};
    })(),
  };
}
    ...
    if (isLayoutContainer) {
      ...
      const hasIconChild = Array.from(el.children).some((child) => {
        if (child.tagName === "I") {
          ...
          return (
            childCls.includes("fa-") || childCls.includes("fas") || childCls.includes("far") || childCls.includes("fab")
          );
        }
        return false;
      });
...
export function generateGradientSVG(w, h, bgString, radius, border) {
  try {
    ...
    parsedStops.forEach((stop, idx) => {
      ...
      const isTransparent =
        color.toLowerCase() === "transparent" || color === "rgba(0, 0, 0, 0)" || color === "rgba(0,0,0,0)";
  ...
  for (let i = 0; i < charToDomMap.length; i++) {
    ...
    if (prevTop !== null) {
      ...
      if (isNewLine) {
        ...
        while (breakPos > 0 && (processedText[breakPos - 1] === " " || processedText[breakPos - 1] === "\t")) {
          breakPos--;
        }
