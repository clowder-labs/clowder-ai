...
function renameTTFInPlace(ttfBuffer, newName) {
  ...
  for (let i = 0; i < numTables; i++) {
    ...
    if (tag === "name") {
      nameTableOffset =
        (result[offset + 8] << 24) | (result[offset + 9] << 16) | (result[offset + 10] << 8) | result[offset + 11];
    } else if (tag === "maxp") {
      maxpOffset =
        (result[offset + 8] << 24) | (result[offset + 9] << 16) | (result[offset + 10] << 8) | result[offset + 11];
    } else if (tag === "loca") {
      locaLength =
        (result[offset + 12] << 24) | (result[offset + 13] << 16) | (result[offset + 14] << 8) | result[offset + 15];
    }
  }
  ...
  // 清零新字符串数据区域（不超过 buffer 边界）
  const writeStart = stringDataStart + newStringDataOffset;
  const maxClearEnd = Math.min(writeStart + 50, result.length);
  for (let i = newNameUtf16.length; writeStart + i < maxClearEnd; i++) {
    result[writeStart + i] = 0;
  }
