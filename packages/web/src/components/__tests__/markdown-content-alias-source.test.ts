...
describe('MarkdownContent alias source', () => {
  it('follows CAT_CONFIGS mentionPatterns dynamically', async () => {
    vi.doMock('@office-claw/shared', async () => {
      ...
      const opusPatterns = [...actual.CAT_CONFIGS.opus.mentionPatterns, '@测试Claude别名'];
      return {
        ...actual,
        CAT_CONFIGS: {
          ...actual.CAT_CONFIGS,
    ...
    const html = renderToStaticMarkup(React.createElement(MarkdownContent, { content: '@测试Claude别名 你先看下' }));
