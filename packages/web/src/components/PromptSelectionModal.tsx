    ...
    >
      <div className="flex w-[min(640px,96vw)] flex-col overflow-hidden rounded-[8px] bg-white shadow-[0_10px_24px_rgba(0,0,0,0.14)]">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2 px-4 pb-4 pt-3">
          ...
          <div className="flex h-[302px] min-h-0 overflow-hidden rounded-[6px] border border-[#E6EAF0] bg-white">
            ...
            <section className="flex min-h-0 flex-1 flex-col overflow-y-auto rounded-[4px] border border-[#E7ECF3] bg-white p-3">
              {selectedItem ? (
                <>
                  <h3 className="text-[12px] font-semibold leading-none text-[#2E3542]">{selectedItem.title}</h3>
                  <div className="mt-1 text-[10px] text-[#8A93A2]">创建人: {selectedItem.creator}</div>
                  <div className="text-[10px] text-[#8A93A2]">创建时间: {selectedItem.createdAt}</div>
                  <div className="mt-2 space-y-2">
                    {buildFallbackSections(selectedItem).map((section) => (
