...
test('isCliTimeoutError matches 响应超时 pattern', () => {
  assert.ok(isCliTimeoutError('Codex CLI 响应超时 (300s)'));
  assert.ok(isCliTimeoutError('Claude CLI 响应超时 (1800s)'));
  assert.ok(isCliTimeoutError('Gemini CLI 响应超时 (300s, 未收到首帧)'));
