...
import { loadEdition, DEFAULT_EDITION, CORE_API_VERSION } from '../dist/edition/edition-loader.js';
...
describe('loadEdition', () => {
  ...
  it('parses edition.json branding overrides', async () => {
    const dir = makeTmpDir();
    writeFileSync(join(dir, 'edition.json'), JSON.stringify({
  ...
  it('rejects path traversal in plugin path', async () => {
    ...
    await assert.rejects(
      ...
      (err) => {
        assert.ok(
          err.message.includes('escapes edition directory'),
