...
describe('BleDeviceManager + BleLimbNode', () => {
  ...
  it('subscribes only to a declared button characteristic and emits typed notifications', async () => {
    const buttonService = '7f9c0001-7d7e-4f1d-9d7b-5f2580000001';
    const buttonCharacteristic = '7f9c0002-7d7e-4f1d-9d7b-5f2580000001';
      ...
      if (command === 'device.inspect') {
        return {
          services: [
            {
              uuid: buttonService,
              characteristics: [{ uuid: buttonCharacteristic, properties: ['notify'] }],
            },
          ],
        };
      }
      if (command === 'gatt.subscribe') return { subscribed: true };
      return originalRequest(command, params);
    };

    const session = await manager.startScan();
    helper.emit('event', discoveryEvent(session.sessionId, 'button-device-id'));
    const binding = await manager.bind({
      sessionId: session.sessionId,
      discoveryId: manager.scanSnapshot().discoveries[0].discoveryId,
    });
    const result = await registry.invoke(binding.nodeId, 'ble.button.subscribe', {}, { catId: 'sol' });
    assert.deepEqual(result, { success: true, data: { subscribed: true } });

    const notificationPromise = new Promise((resolve) => manager.once('notification', resolve));
    helper.emit('event', {
      kind: 'event',
      event: 'gatt.notification',
      data: {
        deviceId: 'button-device-id',
        serviceUuid: buttonService,
        characteristicUuid: buttonCharacteristic,
        valueBase64: Buffer.from([1]).toString('base64'),
        observedAt: 123,
      },
    });
    const notification = await notificationPromise;
    assert.deepEqual(notification.value, { kind: 'button', value: 'press', unit: 'event' });
    assert.equal(notification.bindingId, binding.bindingId);
  });
});
