...
async function seedTemplate(projectRoot) {
  ...
  await writeFile(join(projectRoot, 'office-claw-template.json'), templateRaw, 'utf-8');
}
...
describe('provider profile store', () => {
  ...
  it('creates a client-agnostic api_key account and keeps secrets out of meta', async () => {
    ...
    const metaPath = join(projectRoot, '.office-claw', 'provider-profiles.json');
  ...
  it('does not allow incomplete .git pointers to redirect storage outside the project', async () => {
    ...
    try {
      ...
      const localMetaPath = join(projectRoot, '.office-claw', 'provider-profiles.json');
      const localSecretsPath = join(projectRoot, '.office-claw', 'provider-profiles.secrets.local.json');
      ...
      const escapedMetaPath = join(escapedRoot, '.office-claw', 'provider-profiles.json');
      const escapedSecretsPath = join(escapedRoot, '.office-claw', 'provider-profiles.secrets.local.json');
  ...
  it('migrates project-local profiles to global storage on first read', async () => {
    ...
    process.env.OFFICE_CLAW_GLOBAL_CONFIG_ROOT = globalRoot;
    try {
      const localDir = join(localProject, '.office-claw');
      ...
      const globalMeta = join(globalRoot, '.office-claw', 'provider-profiles.json');
  ...
  it('migration triggers from resolveAnthropicRuntimeProfile (readRaw path)', async () => {
    ...
    try {
      ...
      const meta = JSON.parse(await readFile(join(globalRoot, '.office-claw', 'provider-profiles.json'), 'utf-8'));
      const secrets = JSON.parse(
        await readFile(join(globalRoot, '.office-claw', 'provider-profiles.secrets.local.json'), 'utf-8'),
      );

      await rm(join(globalRoot, '.office-claw'), { recursive: true, force: true });
      await mkdir(localDir, { recursive: true });
  ...
  it('migration triggers from readBootstrapBindingsSync', async () => {
    ...
    try {
      ...
      const meta = await readFile(join(globalRoot, '.office-claw', 'provider-profiles.json'), 'utf-8');
      const secrets = await readFile(join(globalRoot, '.office-claw', 'provider-profiles.secrets.local.json'), 'utf-8');
  ...
  it('merges second project profiles into existing global store', async () => {
    ...
    try {
      ...
      const localDirA = join(projectA, '.office-claw');
      ...
      const globalMeta1 = JSON.parse(await readFile(join(globalRoot, '.office-claw', 'provider-profiles.json'), 'utf-8'));
      assert.ok(
      ...
      const localDirB = join(projectB, '.office-claw');
      ...
      const catalogPathB = join(localDirB, 'office-claw-catalog.json');
      ...
      const localDirC = join(projectC, '.office-claw');
      ...
      await writeFile(
        join(globalRoot, '.office-claw', 'provider-profiles.json'),
        JSON.stringify({
          version: 1,
          providers: {
            anthropic: {
      ...
      await writeFile(
        join(globalRoot, '.office-claw', 'provider-profiles.secrets.local.json'),
        JSON.stringify({
          version: 1,
  ...
  it('rewrites cat catalog accountRef when profile ID is renamed during migration', async () => {
    ...
    try {
      ...
      await writeFile(
        join(localDirB, 'office-claw-catalog.json'),
        JSON.stringify({
          breeds: [
            {
              catId: 'test-cat',
      ...
      const catalogPath = join(localDirB, 'office-claw-catalog.json');
  ...
  it('skips re-migration when project root is recorded in global migrated-roots', async () => {
    ...
    try {
      ...
      const localDir = join(project, '.office-claw');
      await mkdir(localDir, { recursive: true });
      await writeFile(
        join(localDir, 'provider-profiles.json'),
        JSON.stringify({
