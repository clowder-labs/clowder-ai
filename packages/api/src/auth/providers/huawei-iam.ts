import type { AuthenticateOutcome, AuthProvider, AuthSessionInfo } from '@cat-cafe/plugin-api/auth';
...
export function createHuaweiIamAuthProvider(options: HuaweiIamAuthProviderOptions = {}): AuthProvider {
  ...
  return {
     ...
     * Does NOT perform MaaS subscription — that's in postLoginInit.
     */
    ...
    /**
     * Post-login initialization: MaaS subscription + model refresh.
     * Called by platform AFTER session is issued.
     * Failure here does NOT roll back the successful authentication.
     */
    async postLoginInit(session: AuthSessionInfo): Promise<void> {
      const state = session.providerState as HuaweiProviderState | undefined;
      if (!state?.token) return;

      const modelInfoResult = await subscriptionClaw(fetchImpl, state.token, lastPromotionCode, state.promotionCode);

      if (modelInfoResult.success) {
        if (state.promotionCode) {
          lastPromotionCode = state.promotionCode;
        }
        state.modelInfo = modelInfoResult.modelInfo ?? {};
      } else {
        // Log but do NOT fail — auth already succeeded
        console.warn('Huawei MaaS subscription failed (non-fatal):', modelInfoResult.message);
        if (modelInfoResult.needCode) {
