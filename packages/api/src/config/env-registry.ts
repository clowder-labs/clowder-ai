...
/** Shared boolean convention for settings whose consumers accept `1` or `true`. */
...
export interface EnvDefinition {
  ...
  /** User-facing label in the curated System Settings projection. */
  label?: string;
  /** Display group in the curated System Settings projection. */
  settingsGroup?: SettingsGroupKey;
  /** Whether the effective runtime value changes only after a process restart. */
  restartRequired?: boolean;
  /** Runtime truthiness semantics for read-only boolean status rows. */
  booleanSemantics?: {
    defaultOn: boolean;
...
export const ENV_VARS: EnvDefinition[] = [
  ...
  {
    name: 'CAT_CAFE_NODE_MIN_MAJOR',
    defaultValue: '24',
    description: 'Node runtime guard 最低允许主版本（安装/验证脚本启动时读取）',
    category: 'server',
    sensitive: false,
    runtimeEditable: false,
  },
  {
    name: 'CAT_CAFE_NODE_MAX_MAJOR_EXCLUSIVE',
    defaultValue: '26',
    description: 'Node runtime guard 排他的最高主版本边界（安装/验证脚本启动时读取）',
    category: 'server',
    sensitive: false,
    runtimeEditable: false,
  },
  {
    name: 'CAT_CAFE_NODE_PINNED_MAJOR',
    defaultValue: '24',
    description: '启动脚本优先 re-exec 到的 Node 主版本；用于避免 Homebrew/node alias 自动漂移',
    category: 'server',
    sensitive: false,
    runtimeEditable: false,
  },
  {
    name: 'CAT_CAFE_NODE_PREFERRED_MAJORS',
    defaultValue: '24 25',
    description: '启动脚本搜索本机 Node runtime 的主版本候选顺序（空格分隔）',
    category: 'server',
    sensitive: false,
    runtimeEditable: false,
  },
  {
    name: 'CAT_CAFE_NODE_BIN',
    defaultValue: '(未设置)',
    description: '启动脚本优先使用的 Node 可执行文件绝对路径；用于多 Node 环境或手动指定受支持 runtime',
    category: 'server',
    sensitive: false,
    runtimeEditable: false,
  },
  {
    name: 'CAT_CAFE_SKIP_NODE_RUNTIME_GUARD',
    defaultValue: '0',
    description: '跳过 Node runtime guard 的安装期应急开关；仅在明确知道风险时使用',
    category: 'server',
    sensitive: false,
    runtimeEditable: false,
  },
  {
    name: 'CAT_CAFE_SKIP_PRODUCTION_INSTALL_GUARD',
    defaultValue: '0',
    description: '仅跳过 production install guard，保留 Node 版本检查；供验证脚本绕过安装模式检查使用',
    category: 'server',
    sensitive: false,
    runtimeEditable: false,
  },
  ...
  {
    name: 'CAT_CAFE_SUPERVISOR_SOCKET_DIR',
    defaultValue: '(内部注入)',
    description: 'CLI supervisor owner manifest 绑定的 Codex Unix socket 目录',
    category: 'cli',
    sensitive: false,
    hubVisible: false,
    runtimeEditable: false,
  ...
  {
    name: 'CAT_CAFE_PROCESS_OWNER_ID',
    defaultValue: '(内部注入)',
    description: 'CLI 子进程树继承的随机 ownership token，不需要手动设置',
    category: 'cli',
    sensitive: true,
    hubVisible: false,
    runtimeEditable: false,
