...
/**
 * F253 Phase C — qc-metrics-rollup sourceRefs. Replayable QC metrics rollup
 * window selector: provider resolves to zero-baseline QcMetricsSnapshot (Phase C
 * bootstrap) or live aggregated review metrics (future phases). Generator writes
 * snapshot + attribution + provenance into bundle.
 *
...
const qcMetricsSourceRefsShape = z
  ...
  .describe('eval:qc sourceRefs — replayable QC metrics rollup window selector (window start/end).');
