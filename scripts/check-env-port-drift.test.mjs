...
`;
  ...
  it('sync-to-opensource.sh leaves sync tag publication to scripts/publish-sync-tag.sh', () => {
    ...
    assert.match(
      publishScript,
      /ensure_tag_points_to "\$SOURCE_DIR" "office-claw" "\$SOURCE_SHA"/,
  ...
  it('sync-hotfix.sh selects the latest sync baseline by mirrored target tag commit time', () => {
    ...
    assert.match(
      ...
      'hotfix lane should refresh OfficeClaw sync tags from origin before auto-selecting the baseline',
    );
    assert.match(
      hotfix,
...
describe('Sync validation enforces static quality gates', {
  skip: !isHomeRepo && 'sync infrastructure not present (open-source repo)',
}, () => {
  ...
  it('temp target public gate appends the validation checkout to PROJECT_ALLOWED_ROOTS', () => {
    ...
    assert.match(
      gate,
      /PROJECT_ALLOWED_ROOTS_APPEND=true[\s\\]+PROJECT_ALLOWED_ROOTS="\$gate_target_real"[\s\\]+pnpm --filter @office-claw\/api run test:public/,
  ...
  it('temp target public gate preserves full test:public output before tailing', () => {
    ...
    assert.match(
      gate,
      /test_public_log=\$\(mktemp "\$\{TMPDIR:-\/tmp\}\/office-claw-testpublic\.XXXXXX"\)/,
      'run_target_public_gate should capture test:public output in a dedicated temp log',
    );
    assert.match(
      gate,
      /pnpm --filter @office-claw\/api run test:public >"\$test_public_log" 2>&1/,
    ...
    assert.doesNotMatch(
      gate,
      /pnpm --filter @office-claw\/api run test:public 2>&1 \| tail -5/,
      'test:public should not pipe directly into tail, or failures become opaque',
    );
  });
});

describe('Sync runtime-safety guards stay source-side and shell-safe', {
  skip: !isHomeRepo && 'sync infrastructure not present (open-source repo)',
}, () => {
...
describe('Public-facing skill docs avoid home-only API defaults', {
  skip: !isHomeRepo && 'sync infrastructure not present (open-source repo)',
}, () => {
